...
async function reminderMail(d) {
  const cancelTok = P.cancelToken(d.memberId, d.date);   // null, falls kein Storno-Secret gesetzt
  ...
  const mail = renderEmail({
    ...
    secondary: cancelUrl ? { label: 'Doch keine Zeit? Stornieren', href: cancelUrl } : undefined,
    promo: true,
    footer: 'member',
  });
  return {
