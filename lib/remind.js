...
function reminderMail(d) {
  const name = d.firstName ? (' ' + d.firstName) : '';
  const cancelUrl = 'https://mitglieder.fit-inn-trier.de/api/plan-cancel?t=' +
    encodeURIComponent(P.cancelToken(d.memberId, d.date));
  return {
    to: d.email,
    subject: 'Erinnerung: Dein Training heute um ' + fmtSlot(d.slot) + ' Uhr',
    text:
      'Hallo' + name + ',\n\n' +
      'kurze Erinnerung: Du hast dir heute um ' + fmtSlot(d.slot) + ' Uhr ein Training im Fit-Inn Trier vorgemerkt.\n' +
      'Bis gleich! 💪\n\n' +
...
async function dispatch(opts) {
  ...
  for (const d of due) {
    if (!d.email) continue;
    try {
      const r = await sendMailRaw(reminderMail(d));
