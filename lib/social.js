...
async function snapshot(id) {
  ...
  const [buddies, requests, unread, plans] = await Promise.all([listBuddies(id), listRequests(id), unreadMap(id), listPlans(id)]);
  return { ok: true, available: true, enabled: true, profile: publicMe(me), buddies: buddies, requests: requests, unread: unread, plans: plans };
}
