...
const MEMBERS_SET = 'soc:members';               // SET aller opt-in-Mitglieder (Team-Statistik + Cron-Iteration)
const PLANS_ALL = 'soc:plans:all';               // SET aller offenen Trainingsverabredungen (für den Erinnerungs-Cron)
...
// Testen gedacht, solange das Feature noch nicht öffentlich ist. OPT-IN über
// die Umgebungsvariable SOCIAL_DEMO=1 (nur Staging) – in Produktion sind
// Demo-Mitglieder/Testdaten damit standardmäßig deaktiviert.
const DEMO_ON = process.env.SOCIAL_DEMO === '1';
...
async function enable(id, member, opts) {
  ...
  await saveMe(id, me);
  try { await redisPipeline([['SADD', MEMBERS_SET, id]]); } catch (e) {}
  try { await require('./privacy').recordConsent(id, 'community', true, { source: 'community-onboarding' }); } catch (e) {}   // Register für Team-Statistik + Cron
...
async function disable(id) {
  ...
  await saveMe(id, me);
  try { await redisPipeline([['SREM', MEMBERS_SET, id]]); } catch (e) {}
  try { await require('./privacy').recordConsent(id, 'community', false, { source: 'community-off' }); } catch (e) {}
  return { ok: true };
}
async function erase(id) {
  id = sid(id);
  await disable(id);
  try { await redisPipeline([['DEL', meKey(id)], ['SREM', MEMBERS_SET, id], ['SREM', BANNED_SET, id]]); } catch (e) {}
  return { ok: true };
}
...
// ── Vitalpunkte-Ledger (Trainingspartner → globale Vitalpunkte, analog Ernährung) ──
// Stabile Ereignisse (neue Verbindung, abgeschlossene gemeinsame Session) geben VP;
// der Client faltet diese Einträge in dieselbe Punktesumme wie Check-ins/Ernährung.
const VPLEDGER_MAX = 120;
const vpLedgerKey = (id) => 'soc:vpledger:' + id;
function ymdNow() { return ymd(new Date()); }
async function ledgerAdd(id, date, pts, reason) {
  id = sid(id); pts = parseInt(pts, 10) || 0;
  if (!hasStore || !id || isDemo(id) || pts <= 0) return;
  try { await redisPipeline([['LPUSH', vpLedgerKey(id), JSON.stringify({ date: date || ymdNow(), pts: pts, reason: String(reason || '').slice(0, 60) })], ['LTRIM', vpLedgerKey(id), 0, VPLEDGER_MAX - 1]]); } catch (e) {}
}
async function vpLedger(id) {
  id = sid(id); if (!hasStore || !id) return [];
  let raw; try { [raw] = await redisPipeline([['LRANGE', vpLedgerKey(id), 0, VPLEDGER_MAX - 1]]); } catch (e) { return []; }
  if (!Array.isArray(raw)) return [];
  const out = []; raw.forEach(function (s) { try { const o = JSON.parse(s); if (o && (o.pts | 0) > 0) out.push({ date: o.date || '', pts: o.pts | 0, reason: o.reason || '' }); } catch (e) {} });
  return out;
}
// Einmalig VP je Paar bei neuer Verbindung (per NX-Marker gegen Doppelvergabe).
async function awardConnectVP(a, b) {
  a = sid(a); b = sid(b);
  if (!hasStore || !a || !b || isDemo(a) || isDemo(b)) return;
  const mk = 'soc:vpc:' + (a <= b ? (a + ':' + b) : (b + ':' + a));
  try { const [set] = await redisPipeline([['SET', mk, '1', 'NX']]); if (!set) return; } catch (e) { return; }
  const day = ymdNow();
  await ledgerAdd(a, day, 30, 'Neuer Trainingspartner');
  await ledgerAdd(b, day, 30, 'Neuer Trainingspartner');
}
...
async function doConnect(a, b) {
  ...
  try {
    ...
    try { await awardConnectVP(a, b); } catch (e) {}
    return true;
...
async function proposePlan(id, buddyId, date, slot) {
  ...
  // Für den Erinnerungs-Cron registrieren (Demo-Verabredungen ausgenommen – kein echter Partner).
  if (!isDemo(buddyId)) { try { await redisPipeline([['SADD', PLANS_ALL, key]]); } catch (e) {} }
...
// ── F3: Erinnerungen (Trainingsverabredung) + Wochen-Challenge-Ergebnis (Cron) ──
// Aufgerufen aus api/social-remind.js (GitHub-Actions-Cron, alle 15 Min). Wirft nie.
const REMIND_LEAD = 100 * 60 * 1000;          // bis zu ~100 Min vor der Zeit erinnern
const SESSION_DONE_AFTER = 3 * 3600 * 1000;   // danach gilt die Session als gelaufen -> VP
const SESSION_VP = 40;
async function listMembers() { if (!hasStore) return []; return membersOf(MEMBERS_SET); }
// Wall-Clock der Verabredung. Studio-Zeitzone (Europe/Berlin) grob über festen Offset,
// damit „18:00" nicht als UTC interpretiert wird. Genügt für eine ~100-Min-Erinnerung.
const TZ_OFFSET_MIN = parseInt(process.env.SOCIAL_TZ_OFFSET_MIN || '60', 10);
function planStartMs(date, slot) {
  const base = Date.parse(String(date) + 'T00:00:00Z');
  if (isNaN(base)) return NaN;
  return base + ((parseInt(slot, 10) || 0) - TZ_OFFSET_MIN) * 60000;
}
function pushSession(rec, whenLabel) {
  if (!Push || !Push.notifyMember) return;
  const members = [sid(rec.by), sid(rec.buddy)];
  members.forEach(function (mid) {
    const other = mid === sid(rec.by) ? sid(rec.buddy) : sid(rec.by);
    loadMe(other).then(function (om) {
      const nm = (om && om.displayName) || 'deinem Trainingspartner';
      try { Push.notifyMember(mid, 'pushSocial', { title: 'Training heute', body: whenLabel + ': Training mit ' + nm + ' 💪', url: '/mitglieder?go=social&chat=' + encodeURIComponent(other) }); } catch (e) {}
    }).catch(function () {});
  });
}
// Fällige Verabredungen: erinnern (~100 Min vorher, einmalig) + nach der Zeit VP gutschreiben.
async function remindDue(nowMs) {
  if (!hasStore) return { due: 0, sent: 0, credited: 0 };
  nowMs = nowMs || Date.now();
  let keys; try { [keys] = await redisPipeline([['SMEMBERS', PLANS_ALL]]); } catch (e) { return { due: 0, sent: 0, credited: 0 }; }
  keys = Array.isArray(keys) ? keys : [];
  let due = 0, sent = 0, credited = 0;
  for (const key of keys) {
    let rec = null; try { const [s] = await redisPipeline([['GET', key]]); if (s) rec = JSON.parse(s); } catch (e) {}
    if (!rec) { try { await redisPipeline([['SREM', PLANS_ALL, key]]); } catch (e) {} continue; }
    const both = (rec.joined || []).length >= 2;
    const start = planStartMs(rec.date, rec.slot);
    if (isNaN(start) || isDemo(rec.by) || isDemo(rec.buddy)) { try { await redisPipeline([['SREM', PLANS_ALL, key]]); } catch (e) {} continue; }
    if (!both) { if (nowMs > start + SESSION_DONE_AFTER) { try { await redisPipeline([['SREM', PLANS_ALL, key]]); } catch (e) {} } continue; }
    if (nowMs >= start - REMIND_LEAD && nowMs < start) {
      due++;
      let set; try { [set] = await redisPipeline([['SET', 'soc:planremind:' + key, '1', 'NX', 'EX', String(6 * 3600)]]); } catch (e) {}
      if (set) { try { pushSession(rec, 'Gleich'); } catch (e) {} sent++; }
    }
    if (nowMs >= start + SESSION_DONE_AFTER) {
      let set2; try { [set2] = await redisPipeline([['SET', 'soc:planvp:' + key, '1', 'NX']]); } catch (e) {}
      if (set2) { await ledgerAdd(rec.by, rec.date, SESSION_VP, 'Gemeinsames Training'); await ledgerAdd(rec.buddy, rec.date, SESSION_VP, 'Gemeinsames Training'); credited++; }
      try { await redisPipeline([['SREM', PLANS_ALL, key]]); } catch (e) {}
    }
  }
  return { due: due, sent: sent, credited: credited };
}
// Wochen-Challenge-Ergebnis: sonntags im Abendfenster je aktivem Mitglied Platzierung pushen.
async function weeklyResults(nowMs) {
  if (!hasStore) return { pushed: 0 };
  nowMs = nowMs || Date.now();
  const d = new Date(nowMs); const dow = d.getUTCDay(); const hourLocal = d.getUTCHours() + Math.floor(TZ_OFFSET_MIN / 60);
  if (dow !== 0 || hourLocal < 18 || hourLocal >= 21) return { pushed: 0, skipped: true };
  const stamp = 'soc:weekres:' + d.getUTCFullYear() + '-' + d.getUTCMonth() + '-' + d.getUTCDate();
  let owns; try { [owns] = await redisPipeline([['SET', stamp, '1', 'NX', 'EX', String(4 * 3600)]]); } catch (e) {}
  if (!owns) return { pushed: 0, done: true };
  let M = null; try { M = require('./members'); } catch (e) {}
  const ids = await listMembers(); let pushed = 0;
  const weekOf = async function (mid) { try { const r = await M.recentCheckins(mid, { windows: 2 }); const ci = Array.isArray(r) ? r : ((r && r.checkins) || []); return checkinVitals(ci).weekVisits; } catch (e) { return 0; } };
  for (const id of ids) {
    try {
      const buddies = await membersOf(connKey(id)); if (!buddies.length) continue;
      const mine = await weekOf(id);
      let ahead = 0, total = 1;
      for (const b of buddies) { if (isDemo(b)) continue; total++; const bv = await weekOf(b); if (bv > mine) ahead++; }
      if (total < 2) continue;
      const place = ahead + 1;
      if (Push && Push.notifyMember) { try { Push.notifyMember(id, 'pushSocial', { title: 'Wochen-Challenge', body: 'Du hast die Woche auf Platz ' + place + ' von ' + total + ' beendet 🏅 – neue Runde startet!', url: '/mitglieder?go=social' }); } catch (e) {} pushed++; }
    } catch (e) {}
  }
  return { pushed: pushed };
}
// Vom Cron aufgerufen: Erinnerungen immer, Ergebnis nur im Sonntagsfenster.
async function dispatchReminders(opts) {
  opts = opts || {};
  const now = opts.now || Date.now();
  const rem = await remindDue(now).catch(function () { return { due: 0, sent: 0, credited: 0 }; });
  const wk = await weeklyResults(now).catch(function () { return { pushed: 0 }; });
  return { ok: true, reminders: rem, weekly: wk };
}
...
// Nutzungs-Kennzahlen für das Team-Backend (aus dem Opt-in-Register + Verbindungen).
async function communityStats() {
  if (!hasStore) return { enabled: 0, connections: 0, withBuddies: 0, plannedSessions: 0 };
  const ids = await listMembers();
  let connSum = 0, withBuddies = 0;
  for (const id of ids) {
    let c; try { [c] = await redisPipeline([['SCARD', connKey(id)]]); } catch (e) { c = 0; }
    c = Number(c) || 0; connSum += c; if (c > 0) withBuddies++;
  }
  let plans; try { [plans] = await redisPipeline([['SMEMBERS', PLANS_ALL]]); } catch (e) { plans = []; }
  return { enabled: ids.length, connections: Math.round(connSum / 2), withBuddies: withBuddies, plannedSessions: Array.isArray(plans) ? plans.length : 0 };
}
...
async function snapshot(id) {
  ...
  const [buddies, requests, unread, plans, rooms, vitalLedger] = await Promise.all([listBuddies(id), listRequests(id), unreadMap(id), listPlans(id), listRooms(id), vpLedger(id)]);
  return { ok: true, available: true, enabled: true, profile: publicMe(me), buddies: buddies, requests: requests, unread: unread, plans: plans, rooms: rooms, vitalLedger: vitalLedger, demoCode: DEMO_ON ? DEMO_CODE : null };
}

module.exports = {
  MIN_AGE, MAX_BUDDIES,
  ageOk, ageFrom, initialsOf,
  enable, disable, erase, setShare,
  ...
  nutriStreakOf, cheer,
  vpLedger, ledgerAdd, awardConnectVP,
  listMembers, remindDue, weeklyResults, dispatchReminders,
  socialEnabled, setSocialEnabled, isBanned, setBan, listBanned, listReports, clearReports, communityStats, socialExport,
  loadMe, snapshot,
};
