...
async function snapshot(id) {
  ...
  const [buddies, requests] = await Promise.all([listBuddies(id), listRequests(id)]);
  return { ok: true, available: true, enabled: true, profile: publicMe(me), buddies: buddies, requests: requests };
}
