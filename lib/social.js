...
async function nutriStreakOf(id) {
  id = sid(id); if (!hasStore) return 0;
...
async function cheer(id, buddyId) {
  ...
  if (Push && Push.notifyMember) { loadMe(id).then(function (me) { const nm = (me && me.displayName) || 'Dein Trainingspartner'; try { Push.notifyMember(buddyId, 'pushSocial', { title: nm, body: 'feuert dich an! 💪 Auf geht\'s!', url: '/mitglieder?go=social' }); } catch (e) {} }).catch(function () {}); }
  return { ok: true };
}
...
async function proposePlan(id, buddyId, date, slot) {
  ...
  const rec = { by: id, buddy: buddyId, date: date, slot: slot, joined: [id], at: Date.now() };
...
async function nameMapFor(memberIds) {
  ...
  memberIds.forEach(function (m, i) { let bme = {}; try { bme = JSON.parse(mes[i]) || {}; } catch (e) {} out[sid(m)] = bme.displayName || 'Mitglied'; });
  return out;
}
...
function pushRoom(fromId, memberIds, name, text) {
  if (!Push || !Push.notifyMember) return;
  loadMe(fromId).then(function (me) {
    const nm = (me && me.displayName) || 'Trainingspartner';
    (memberIds || []).forEach(function (mid) { if (sid(mid) === sid(fromId)) return; try { Push.notifyMember(mid, 'pushSocial', { title: name || 'Gruppe', body: text || (nm + ': neue Nachricht'), url: '/mitglieder?go=social' }); } catch (e) {} });
  }).catch(function () {});
}
async function createRoom(id, name, memberIds) {
  ...
  try { pushRoom(id, members, name, 'Du wurdest zur Gruppe „' + name + '" hinzugefügt.'); } catch (e) {}
...
async function roomInvite(id, rid, buddyId) {
  ...
  try { pushRoom(id, [buddyId], room.name, 'Du wurdest zur Gruppe „' + room.name + '" hinzugefügt.'); } catch (e) {}
  return { ok: true };
}
...
async function roomSend(id, rid, text) {
  ...
  try { await redisPipeline(cmds); } catch (e) { return { ok: false, error: 'server_error' }; }
  try { const nameMap = await nameMapFor([id]); pushRoom(id, room.members, room.name, (nameMap[id] || 'Jemand') + ': ' + t.slice(0, 120)); } catch (e) {}
...
async function snapshot(id) {
  ...
  return { ok: true, available: true, enabled: true, profile: publicMe(me), buddies: buddies, requests: requests, unread: unread, plans: plans, rooms: rooms };
}
