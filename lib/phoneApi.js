...
async function guard(req, body, limit) {
  ...
  if (!secret) return { ok: false, code: 503, body: { ok: false, error: 'not_configured', text: 'Die Telefon-Schnittstelle ist nicht eingerichtet.' } };
  if (!sameSecret(keyFrom(req, body), secret)) return { ok: false, code: 401, body: { ok: false, error: 'unauthorized' } };
