...
const SLOT_MIN = 30;
// Nur aus der Umgebung – kein festes Fallback-Literal (öffentliches Repo!).
// Ohne Secret werden keine Storno-Tokens erzeugt und Verifizierung schlägt fehl.
const CANCEL_SECRET = process.env.PLAN_CANCEL_SECRET || process.env.RECORD_SECRET || '';
const hasCancelSecret = !!CANCEL_SECRET;
...
function cancelToken(memberId, date) {
  if (!hasCancelSecret) return null;
  const mid = String(memberId);
  return mid + '.' + date + '.' + sign(mid, date);
}
function verifyCancelToken(token) {
  if (!hasCancelSecret) return null;   // fail-closed ohne Secret
