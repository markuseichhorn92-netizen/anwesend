...
async function setPlan(memberId, minutes, opts, now) {
  ...
  if (slot < nowMin) return { ok: false, error: 'past' }; // keine vergangene Zeit heute
  ...
  return { ok: true, plan: { date: date, slot: slot, remind: remind } };
}
...
async function cancelPlan(memberId, now) {
  if (!hasStore) return { ok: false, error: 'no_store' };
  return cancelForDate(memberId, parts(now).date);
}

async function getMyPlan(memberId, now) {
  if (!hasStore) return null;
  const pn = parts(now), nowMin = pn.hour * 60 + pn.minute;
  const [raw] = await redisPipeline([['HGET', K.by(pn.date), String(memberId)]]);
  if (!raw) return null;
  try {
    const o = JSON.parse(raw);
    const slot = recSlot(o);
    if (slot === null || !slotActive(slot, nowMin)) return null;   // abgelaufen -> verschwindet
    return { slot: slot, remind: o.remind !== false };
  } catch (e) { return null; }
}

// Öffentliche, anonyme Aggregation: pro Stunde summiert (+ pro Slot) – NUR aktive Slots.
async function getAggregate(now) {
  ...
  Object.keys(m).forEach((k) => {
    ...
    if (!slotActive(slot, nowMin)) return;                 // abgelaufen -> nicht mehr anzeigen
    const h = Math.floor(slot / 60);
    hours[h] = (hours[h] || 0) + c;
    slots[slot] = (slots[slot] || 0) + c;
    total += c;
  });
  return { available: true, date: pn.date, hours: hours, slots: slots, total: total };
}
