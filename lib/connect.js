...
async function submitWithdrawal(opts) {
  ...
  return { ok: r.status >= 200 && r.status < 300, status: r.status, json: r.json, text: String(r.text || '').slice(0, 300) };
}
...
async function getTrialSlots(startDate, endDate, trainerRequired) {
  const q = '?studioId=' + encodeURIComponent(STUDIO_ID)
    ...
    + '&trainerRequired=' + (trainerRequired ? 'true' : 'false');
...
async function bookTrial(d) {
  const body = {
    ...
    trainerRequired: !!d.trainerRequired,  // muss zum gewählten Slot passen (Slots werden mit demselben Flag geholt)
    note: d.referralCode
