...
async function metaSend(payload) {
  try {
    ...
    let id = null; try { const j = JSON.parse(body); id = (j && j.messages && j.messages[0] && j.messages[0].id) || null; } catch (e) {}
...
async function twilioSendText(toNum, text) {
  try {
    ...
    let id = null; try { const j = JSON.parse(t); id = (j && j.sid) || null; } catch (e) {}
    return { ok: r.ok, status: r.status, body: t.slice(0, 300), id: id };
  } catch (e) { return { ok: false, error: e.message }; }
}
...
module.exports = {
  ...
  verifyChallenge, verifySignature, parseInbound, parseStatuses,
