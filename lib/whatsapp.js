...
// Die erkannte Form ueberdauert den Prozess. Sonst faengt jede frisch
// gestartete Funktion wieder bei Null an und bezahlt einen abgelehnten Aufruf
// - also genau die Verzoegerung, ueber die sich hier zuerst beschwert wurde.
const FORM_KEY = 'otptplform';
const FORM_TTL = 30 * 24 * 3600;
let formGeladen = !!WA_LOGIN_FORCE;
let formGemerkt = WA_LOGIN_FORCE;

async function formLaden() {
  if (formGeladen || !hasStore) { formGeladen = true; return; }
  formGeladen = true;
  try {
    const [v] = await redisPipeline([['GET', FORM_KEY]]);
    if (v === 'auth' || v === 'body') { metaLoginShape = v; formGemerkt = v; }
  } catch (e) {}
}
async function formMerken(shape) {
  if (WA_LOGIN_FORCE || !hasStore || shape === formGemerkt) return;
  formGemerkt = shape;
  try { await redisPipeline([['SET', FORM_KEY, shape, 'EX', String(FORM_TTL)]]); } catch (e) {}
}
...
async function sendLoginTemplate(toNum, code) {
  ...
  if (r && r.ok) { metaLoginShape = zuerst; await formMerken(zuerst); return r; }
  ...
  if (!WA_LOGIN_FORCE && istFormFehler(r)) {
    ...
    if (r2 && r2.ok) { metaLoginShape = andere; await formMerken(andere); return r2; }
    return r2 || r;
  }
  return r;
}
...
// Wie ist der Anmelde-Code-Versand gerade eingerichtet? Nur Ja/Nein und der
// Name der Vorlage - keine Zugangsdaten, keine SIDs. Fuer die Selbstauskunft
// im Team-Backend: ohne sie sieht niemand, ob ueberhaupt etwas eingerichtet ist.
function loginVorlageInfo() {
  return {
    meta: hasMeta,
    twilio: hasTwilio,
    bereit: hasWaLogin,
    anbieter: (hasTwilio && TW_LOGIN_SID) ? 'twilio' : (hasMeta && WA_LOGIN_TPL ? 'meta' : null),
    name: (hasTwilio && TW_LOGIN_SID) ? null : (WA_LOGIN_TPL || null),
    sprache: WA_LOGIN_LANG,
    form: metaLoginShape,          // welche Form zuletzt funktioniert hat
    festgenagelt: WA_LOGIN_FORCE,  // per WA_LOGIN_TEMPLATE_AUTH erzwungen
  };
}
...
module.exports = {
  ...
  fehlerInfo, loginComponents, loginVorlageInfo,
