...
async function registerToken(memberId, token, platform) {
  ...
  await redisPipeline([
    ...
    ['SADD', 'push:members', String(memberId)],
  ]);
  return true;
}
...
async function registerTeamToken(token, platform, who) {
  if (!hasStore || !token) return false;
  await redisPipeline([
    ['SADD', TEAM_TOKENS, String(token)],
    ['SET', teamMetaKey(token), JSON.stringify({ platform: String(platform || ''), who: String(who || '').slice(0, 80), at: Date.now() })],
  ]);
  return true;
}
