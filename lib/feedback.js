...
const CAMPAIGN_KEY = 'fb:campaign';   // aktive „Bitte um Feedback"-Aktion (studioweit, ein Datensatz)
...
// ── Feedback-Aktion (Kampagne) ──────────────────────────────────────────────
// Eine studioweite, einfache „Bitte um Feedback": Das Team startet sie (und löst
// zugleich einen Push an alle App-Nutzer aus). Beim nächsten App-Start erscheint
// sie EINMAL als Feedback-Fenster. Dass ein Nutzer sie gesehen hat, merkt sich
// der Client nur LOKAL (kein Personenbezug), passend zur anonymen Feedback-Logik.
async function getCampaign() {
  if (!hasStore) return null;
  try { const [v] = await redisPipeline([['GET', CAMPAIGN_KEY]]); return v ? (typeof v === 'string' ? JSON.parse(v) : v) : null; }
  catch (e) { return null; }
}
async function startCampaign(input) {
  ...
  const rec = {
    id: newId(),
    title: str(input.title, 80) || 'Deine Meinung zählt',
    text: textField(input.text, 400) || 'Wir würden uns über dein kurzes Feedback zur App sehr freuen.',
    createdAt: nowMs(),
  };
  if (!hasStore) return { ok: false, disabled: true };
  try { await redisPipeline([['SET', CAMPAIGN_KEY, JSON.stringify(rec)]]); return { ok: true, campaign: rec }; }
  catch (e) { return { ok: false, error: 'store_failed' }; }
}
async function stopCampaign() {
  if (!hasStore) return { ok: false, disabled: true };
  try { await redisPipeline([['DEL', CAMPAIGN_KEY]]); return { ok: true }; }
  catch (e) { return { ok: false, error: 'store_failed' }; }
}
...
module.exports = {
  ...
  getCampaign, startCampaign, stopCampaign,
};
