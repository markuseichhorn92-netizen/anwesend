...
async function setOptIn(id, on) {
  ...
  const out = await save(id, next);
  try { await require('./privacy').recordConsent(id, 'finn_memory', !!on, { source: 'finn-memory' }); } catch (e) {}
  return out;
}
