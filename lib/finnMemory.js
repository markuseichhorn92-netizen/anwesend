...
function normalize(obj) {
  ...
  return { on: !!o.on, items: items };
}

async function get(id) {
  if (!hasStore || !id) return { on: false, items: [] };
  try {
    const [v] = await redisPipeline([['GET', MKEY(id)]]);
    if (!v) return { on: false, items: [] };
    return normalize(typeof v === 'string' ? JSON.parse(v) : v);
  } catch (e) { return { on: false, items: [] }; }
}
...
async function save(id, state) {
  ...
  try {
    await redisPipeline([['SET', MKEY(id), JSON.stringify({ v: 1, on: clean.on, items: clean.items })]]);
    try { await redisPipeline([['EXPIRE', MKEY(id), String(TTL)]]); } catch (e) {}
  } catch (e) {}
  return clean;
}

// Opt-in setzen. AUS ⇒ gemerkte Fakten werden sofort gelöscht („aus = vergessen").
async function setOptIn(id, on) {
  const cur = await get(id);
  const next = { on: !!on, items: on ? cur.items : [] };
  return save(id, next);
}
...
async function remember(id, memos) {
  ...
  return save(id, { on: true, items: items });
}
...
async function forget(id, text) {
  ...
  return save(id, { on: cur.on, items: items });
}

// Alles Gemerkte löschen (Opt-in-Status bleibt).
async function clear(id) {
  const cur = await get(id);
  return save(id, { on: cur.on, items: [] });
}
