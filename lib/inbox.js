...
async function reply(memberId, id, text) {
  ...
  // Mitglied hat geantwortet -> Team muss ran.
  v.teamStatus = 'neu';
  v.teamUnread = true;
...
async function teamReply(memberId, id, text, opts) {
  ...
  try {
    ...
    await Push.notifyMember(memberId, 'pushPostfach', {
      ...
      body: (v.subject ? (v.subject + ': ') : '') + 'Das Team hat dir geantwortet.',
      url: 'https://mitglieder.fit-inn-trier.de/mitglieder',
    });
  } catch (e) {}
  return v;
}
...
module.exports = {
  addVorgang, list, get, markRead, reply, teamReply, resolve, unreadCount, reassign, merge,
  listAll, setMeta, addNote, markTeamRead, setMemberSnapshot,
