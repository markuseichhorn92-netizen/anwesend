...
async function handleInbound(o) {
  ...
  const M = require('./members');
  const Inbox = require('./inbox');
  const quietLog = () => Inbox.reply(memberId, v.id, msg.text, null, { notifyTeam: false }).catch(function () {});
  ...
  // 2) Verifiziert: heikle/nach außen wirkende Themen -> Übergabe an einen Menschen (E-Mail ans Team).
