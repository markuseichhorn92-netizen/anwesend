...
async function handleInbound(o) {
  ...
  if (!verified) {
    ...
    await ownerReply(memberId, v.id,
      'Hi! Schön, dass du dich meldest. 😊 Damit ich dir hier sicher zu deinem Vertrag, deinen Terminen und deinem Coaching antworten darf, bestätige bitte einmal kurz, dass du es wirklich bist:\n' + link + '\nDanach beantworte ich deine Fragen direkt hier – dauert nur eine Minute.',
      'System');
    return;
  }
  ...
  // 2b) FINN-Agenten (FINN_AGENTS=1): Routing, Werkzeuge mit Bestätigung per Button, Übergabe
  //     mit Kontext – im bestehenden Vorgang des Threads. Heikle Themen eskaliert der
  //     Orchestrator selbst. Antwortet er nicht (keine KI/Fehler), läuft der bisherige Weg.
  try {
    const Channels = require('./finn/channels');
    if (Channels.agentsOn()) {
      const fa = await Channels.whatsapp({ memberId: memberId, vorgang: v, text: msg.text, phone: phone, history: buildHistory(v) });
      if (fa && fa.handled) {
        try { await require('./opsStat').bump('wa_finn_' + (fa.buttons ? 'confirm' : 'answered')); } catch (e) {}
        if (fa.buttons) { waLog('finn_confirm', { agent: fa.agent }); await ownerChoice(memberId, v.id, fa.buttons.body, fa.buttons.options, fa.text, 'FINN'); }
        else { waLog('finn_answered', { agent: fa.agent }); if (fa.text) await ownerReply(memberId, v.id, fa.text, 'FINN'); }
        return;
      }
      waLog('finn_passthrough', { err: (fa && fa.error) || null });
    }
  } catch (e) { waLog('finn_error', { name: String(e && e.name) }); }
