...
async function readState(scope) {
  ...
  const v = await KV.getJSON(KEY(s));
  const e = v && v.state ? { state: v.state, at: v.at || 0, status: v.status || null, exp: Date.now() + 60000 } : { state: 'unknown', at: 0, status: null, exp: Date.now() + 60000 };
  cache.set(s, e);
  return e;
}
