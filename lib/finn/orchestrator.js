...
const YES = /^\s*(ja|jo|jep|jap|yes|ok|okay|passt|gerne|gern|bitte|best[aä]tige(n)?|mach(e)?\s*das|einverstanden|stimmt|genau|richtig|klar|los)\b[\s!.]*$/i;
const NO = /^\s*(nein|ne|nö|no|nicht|abbrechen|abbruch|stop+|lass(e)?\s*(es|das)|doch\s*nicht|lieber\s*nicht|verwerfen)\b[\s!.]*$/i;
...
async function handle(ctx, input) {
  ...
  const out = (o) => Object.assign({ ok: true, text: '', link: null, confirm: null, handoff: null, agent: null, traceId: ctx.traceId, blocked: false }, o);
  ...
  if (r.escalate) {
    ...
    return out({ agent: 'handoff', handoff: { ref: h.ref }, text: 'Alles klar – da hole ich am besten jemanden aus dem Team dazu. ' + (h.ref ? ('Dein Anliegen liegt unter ' + h.ref + ' im Postfach, ') : '') + 'jemand meldet sich bei dir. 🙌', link: ctx.actor && ctx.actor.kind === 'member' ? { screen: 'postfach', label: 'Postfach öffnen' } : null });
  }
  ...
  return out({ text: res.text, link: res.link, confirm: res.confirm, handoff: res.handoff, agent: res.agent });
}
...
async function confirm(ctx, id, conversationId) {
  ...
  return { ok: !!c.ok, text: resultText(c), agent: ctx.agent || null, traceId: ctx.traceId, done: !!c.ok, error: c.ok ? null : c.error, forbidden: !!c.forbidden, link: null, confirm: null, handoff: null };
}
async function decline(ctx, id, conversationId) {
  ...
  const convId = conversationId != null ? String(conversationId) : actorKey(ctx);
  await Memory.set(ctx.channel || 'web', convId, { pending: null });
  return { ok: true, text: d.ok ? 'Okay, ich habe nichts geändert. Sag einfach, wenn du etwas anderes möchtest.' : U.safeMessage(d.error), agent: ctx.agent || null, traceId: ctx.traceId, declined: !!d.ok, link: null, confirm: null, handoff: null };
}
