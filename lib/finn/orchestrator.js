...
async function handle(ctx, input) {
  ...
  const out = (o) => {
    const r = Object.assign({ ok: true, text: '', link: null, confirm: null, handoff: null, agent: null, traceId: ctx.traceId, blocked: false }, o);
  ...
  return out({ text: res.text, link: res.link, confirm: res.confirm, handoff: res.handoff, agent: res.agent, steps: res.steps, toolsUsed: res.toolsUsed });
}
