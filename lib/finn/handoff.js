...
async function create(ctx, o) {
  ...
  if (actor.kind === 'member' && actor.id != null) {
    try {
      ...
      if (ctx.vorgangId) {
        // WhatsApp/E-Mail: der Thread IST schon ein Vorgang – dort Team-Alarm + Kontextnotiz, kein zweiter Vorgang.
        try { await Inbox.alertTeam(actor.id, ctx.vorgangId); } catch (e) {}
        try { v = await Inbox.addNote(actor.id, ctx.vorgangId, { author: 'FINN', text: text }); } catch (e) {}
        if (!v) { try { v = await Inbox.get(actor.id, ctx.vorgangId); } catch (e) {} }
      }
      if (!v) v = await Inbox.addVorgang(actor.id, {
