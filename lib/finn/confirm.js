...
async function confirm(ctx, id) {
  ...
  return { ok: !!r.ok, result: r, tool: rec.tool, preview: rec.preview, risk: rec.risk, agent: rec.agent || null, validated: r.validated || null, error: r.ok ? null : (r.error || 'failed'), forbidden: !!r.forbidden };
}
...
async function decline(ctx, id) {
  ...
  return { ok: true, tool: rec.tool, preview: rec.preview, agent: rec.agent || null };
}
