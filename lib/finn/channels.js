...
// Antwortvorschläge fürs Frontend: nur Label (+ optional App-Bereich), nie mehr.
function cleanChoices(list) {
  return (Array.isArray(list) ? list : []).slice(0, 4).map((c) => {
    const o = { label: String((c && c.label) || '').slice(0, 40) };
    if (c && c.screen && /^[a-z]+$/.test(String(c.screen))) o.screen = String(c.screen);
    return o;
  }).filter((c) => c.label);
}
function reply(r) {
  return { ok: !!r.ok, answer: r.text || '', link: r.link || null, confirm: r.confirm ? { id: r.confirm.id, preview: r.confirm.preview, risk: r.confirm.risk, expiresAt: r.confirm.expiresAt || null } : null, handoff: r.handoff || null, agent: r.agent || null, traceId: r.traceId || null, error: r.ok ? undefined : (r.error || 'failed'), blocked: !!r.blocked, done: !!r.done, declined: !!r.declined, choices: cleanChoices(r.choices) };
}
...
async function whatsapp(o) {
  ...
  if (r.confirm) {
    ...
    out.text = r.text + '\n\n1) Ja, bestätigen\n2) Abbrechen';
  } else if (r.choicesExplicit && Array.isArray(r.choices) && r.choices.length >= 2) {
    // Rückfrage mit Optionen -> als Quick-Reply-Buttons (Meta: max. 3, Titel ≤ 20 Zeichen);
    // der nummerierte Text ist der Fallback ohne Buttons (Twilio ohne Vorlage).
    const opts = r.choices.slice(0, 3).map((c, i) => ({ id: 'finn:q:' + i, title: String(c.label).slice(0, 20) }));
    out.buttons = { body: r.text, options: opts };
    out.text = r.text + '\n\n' + r.choices.slice(0, 3).map((c, i) => (i + 1) + ') ' + c.label).join('\n');
  }
  return out;
}
