...
// Antwortvorschläge je Agent, wenn das Modell keine eigenen Optionen nennt: die Person
// soll immer auch tippen können, ohne tippen zu müssen. Kurz, konkret, keine
// Aktionen mit Folgen („kündigen") als Ein-Klick-Vorschlag.
const QUICK = {
  concierge: ['Wie lange läuft mein Vertrag?', 'Termin buchen', 'Öffnungszeiten'],
  member: ['Meine Daten zeigen', 'Adresse ändern', 'E-Mail ändern'],
  contract: ['Wann kann ich frühestens kündigen?', 'Beitragspause anfragen', 'Zusatzmodule ansehen'],
  payment: ['Beitragskonto zeigen', 'Bankverbindung ändern', 'Ans Team'],
  appointment: ['Freie Termine zeigen', 'Meine Termine', 'Termin stornieren'],
  access: ['Chip verloren', 'Meine Besuche', 'Mitgliedskarte'],
  document: ['Dokumente zeigen', 'Vertragskopie', 'Ans Team'],
  studio: ['Öffnungszeiten', 'Wie voll ist es gerade?', 'Terminarten'],
  lead: ['Probetraining', 'Was kostet es?', 'Öffnungszeiten'],
  support: ['Login klappt nicht', 'Mitgliedskarte', 'Ans Team'],
  crm: [],
  retention: ['Beitragspause anfragen', 'Mit dem Team sprechen', 'Wann endet mein Vertrag?'],
  handoff: [],
};
function quickFor(key) { return (QUICK[String(key || '')] || []).map((label) => ({ label: label })); }

module.exports = { AGENTS, get, list, forActor, QUICK, quickFor };
