...
const BRAND = [
  ...
  'Antwort-Knöpfe: Wenn du eine Rückfrage stellst oder mehrere Möglichkeiten anbietest (Terminart, Uhrzeiten, Gründe, Ja/Nein), hänge die Auswahl als Marker an: [[antworten: Option 1 | Option 2 | Option 3]] – höchstens 4, je höchstens 5 Wörter, so formuliert, dass die Person sie wörtlich zurückschicken kann. Die App zeigt sie als Knöpfe. Die Optionen im Text nicht noch einmal aufzählen.',
].join('\n');

// Antwortvorschläge aus dem Modelltext ziehen ([[antworten: a | b]]) und den Marker entfernen.
const CHOICE_MAX = 4, CHOICE_LEN = 40;
function extractChoices(text) {
  const out = [];
  const cleaned = String(text || '').replace(/\[\[\s*(?:antworten|optionen|auswahl|choices)\s*:\s*([^\]]*)\]\]/gi, (m, list) => {
    String(list).split('|').map((s) => s.trim().replace(/^[-–•\d.)\s]+/, '').trim()).filter(Boolean).forEach((s) => {
      if (out.length < CHOICE_MAX && !out.some((o) => o.label.toLowerCase() === s.toLowerCase())) out.push({ label: s.slice(0, CHOICE_LEN) });
    });
    return '';
  }).replace(/[ \t]+\n/g, '\n').trim();
  return { text: cleaned, choices: out };
}
...
async function run(o) {
  ...
  // Antwortvorschläge: vom Modell genannt (Marker) oder – ohne Marker – die Standardvorschläge
  // des Agenten. Bei einem Bestätigungsvorschlag keine (die Karte hat ihre eigenen Knöpfe).
  const ch = extractChoices(text); text = ch.text;
  let choices = ch.choices, choicesExplicit = ch.choices.length > 0;
  if (confirm) {
    ...
    choices = []; choicesExplicit = false;
  } else if (handoff) {
    choices = ctx.actor && ctx.actor.kind === 'member' ? [{ label: 'Postfach öffnen', screen: 'postfach' }] : []; choicesExplicit = false;
  } else if (!choices.length) {
    choices = Agents.quickFor(agent.key);
  }
  ...
  return { ok: true, text: text, link: link, confirm: confirm, handoff: handoff, agent: agent.key, steps: steps, toolsUsed: toolsUsed, choices: choices, choicesExplicit: choicesExplicit };
}

module.exports = { run, buildSystem, historyMessages, extractChoices, BRAND, MAX_STEPS };
