...
async function moduleStatus(customerId, opts) {
  ...
  if (res && res.ok && res.contract) {
    out = { booked: true, moduleContractId: cid, endDate: res.contract.endDate || null, cancelled: !!res.contract.cancelled, nextCancellationDate: res.contract.nextCancellationDate || null };
...
async function grantFromBooking(customerId, moduleContractId) {
  ...
  let ent = null; try { ent = await Ent.getEntitlement(customerId); } catch (e) {}
  const now = Date.now();
  // Stripe gewinnt fürs Gating – aber die Modul-Vertrags-ID trotzdem merken, damit ein
  // (zusätzlich) gebuchtes Modul sichtbar & kündbar bleibt (sonst Doppelzahlung unsichtbar).
  if (ent && ent.stripeSubId && Ent.isPremium(ent)) {
    if (moduleContractId != null && String((ent && ent.moduleContractId) || '') !== String(moduleContractId)) {
      try { await Ent.setEntitlement(customerId, Object.assign({}, ent, { moduleContractId: moduleContractId, updatedAt: now })); } catch (e) {}
    }
