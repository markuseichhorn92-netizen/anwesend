...
// Standard-Artikel einmalig in den Store schreiben (idempotent via Flag art:seeded).
// So sind die eingebauten Hilfe-Artikel im Team-Backend sichtbar/bearbeitbar und
// erscheinen – mit echter Aufruf-Zählung – im Mitgliederbereich.
async function seedDefaults() {
  if (!hasStore) return { seeded: 0 };
  let flag, ids;
  try { [flag, ids] = await redisPipeline([['GET', 'art:seeded'], ['SMEMBERS', IDX]]); }
  catch (e) { return { seeded: 0 }; }
  if (flag) return { seeded: 0, skipped: 'flag' };
  if (Array.isArray(ids) && ids.length) { try { await redisPipeline([['SET', 'art:seeded', '1']]); } catch (e) {} return { seeded: 0, skipped: 'existing' }; }
  try { await redisPipeline([['SET', 'art:seeded', '1']]); } catch (e) {}   // zuerst sperren (gegen Doppel-Seed)
  let seed = [];
  try { seed = require('./helpSeed'); } catch (e) { return { seeded: 0 }; }
  let n = 0;
  for (const a of seed) { try { const r = await save({ title: a.title, cat: a.cat, body: a.body, status: a.status }); if (r) n++; } catch (e) {} }
  return { seeded: n };
}
...
module.exports = { list, listPublished, get, save, setStatus, remove, incrView, vote, seedDefaults, hasStore, CATS, CAT_LABEL };
