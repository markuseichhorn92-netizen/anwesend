...
function buildSys() {
  ...
  return [
    ...
    '– Termine: anzeigen (list_appointments), absagen (cancel_appointment), buchen (list_bookable_types → find_appointment_slots → book_appointment). Buche NUR einen konkret bestätigten Slot; sonst 2–3 passende Vorschläge nennen und nachfragen.',
    '– Gewicht/Erfolgskontrolle eintragen (log_weight_checkin).',
 ...
 * Tool-Use-Loop. o: { req, memberId, question }
 * -> { handled:true, text } wenn eine Aktion lief, sonst { handled:false }.
 */
async function tryAction(o) {
  ...
  try {
    const sys = buildSys();
    const convo = [{ role: 'user', content: String(o.question || '').slice(0, 1500) }];
    let used = false;
    for (let step = 0; step < 5; step++) {
      const r = await AI.messagesRaw({ system: sys, messages: convo, tools: ACTION_TOOLS, maxTokens: 700, temperature: 0.1 });
...
module.exports = { tryAction, logFoodPhoto, replyFor, compact, ACTION_TOOLS };
