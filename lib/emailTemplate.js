 ...
 * ------------------------------------
 * Ein Master-Template (premium & minimalistisch) nach dem Design-Handoff:
 * gleicher Header (Teal + Logo), gleiche Bausteine (Eyebrow, Headline,
 * Body, Info-Panel, Code-Block, Button, "Freunde werben"-Promo, Footer) –
 * nur der Inhalt wechselt je nach Anlass.
 *
 * `renderEmail(opts)` liefert { html, text }. HTML ist tabellenbasiert und
 * mit Inline-Styles aufgebaut (maximale Kompatibilität mit Mail-Clients);
 * der Text ist die Klartext-Variante derselben Inhalte (Fallback).
 *
...
function htmlHeader() {
  return '' +
    '<tr><td style="background:' + C.teal + '; padding:26px 40px; text-align:center;">' +
...
function htmlGreeting(name) {
  ...
  return '<div style="font-size:15px; font-weight:700; color:' + C.ink + '; margin-bottom:20px;">' + g + '</div>';
}
...
function htmlHeadline(t) {
  if (!t) return '';
  return '<h1 style="margin:12px 0 0; font-size:26px; font-weight:800; letter-spacing:-.5px; color:' + C.ink + '; line-height:1.22;">' + esc(t) + '</h1>';
}

function htmlParagraphs(intro) {
  return asArray(intro).map(function (p, i) {
    return '<p style="margin:' + (i === 0 ? '16px' : '14px') + ' 0 0; font-size:15px; line-height:1.65; color:' + C.body + '; font-weight:500;">' + esc(p) + '</p>';
  }).join('');
}
...
function htmlPanel(rows) {
  ...
  var inner = rows.map(function (r, i) {
    return '<tr>' +
      '<td style="padding:' + (i ? '13px' : '0') + ' 0 0; font-size:14px; color:' + C.muted + '; font-weight:500;">' + esc(r.label) + '</td>' +
      '<td style="padding:' + (i ? '13px' : '0') + ' 0 0; font-size:14px; color:' + C.ink + '; font-weight:700; text-align:right;">' + esc(r.value) + '</td>' +
      '</tr>';
  }).join('');
  return '<div style="margin-top:24px; background:' + C.panel + '; border-radius:14px; padding:20px 22px;">' +
...
function htmlDivider(t) {
  ...
  return '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%; margin-top:26px;"><tr>' +
    '<td style="width:50%;"><div style="height:1px; background:' + C.hair + '; line-height:1px; font-size:0;">&nbsp;</div></td>' +
    '<td style="white-space:nowrap; padding:0 14px; font-size:11px; font-weight:700; letter-spacing:1px; text-transform:uppercase; color:' + C.fine + ';">' + esc(t) + '</td>' +
    '<td style="width:50%;"><div style="height:1px; background:' + C.hair + '; line-height:1px; font-size:0;">&nbsp;</div></td>' +
    '</tr></table>';
}
...
function htmlCode(code) {
  ...
  return '<div style="margin-top:22px; background:' + C.panel + '; border-radius:14px; padding:24px 24px 22px; text-align:center;">' +
    '<div style="font-size:12px; font-weight:600; color:' + C.muted + '; letter-spacing:.3px;">' + esc(code.label || 'Dein Code') + '</div>' +
    '<div style="margin-top:10px; font-size:' + size + 'px; font-weight:800; letter-spacing:' + spacing + 'px; color:' + C.teal + ';">' + esc(code.value) + '</div>' +
    '</div>';
}

function htmlButton(button, secondary) {
  if (!button || !button.href) {
    // evtl. nur ein sekundärer Link
    if (secondary && secondary.href) {
      return '<div style="margin-top:28px;"><a href="' + esc(secondary.href) + '" style="font-size:14px; font-weight:700; color:' + C.teal + '; text-decoration:none;">' + esc(secondary.label || 'Mehr') + '</a></div>';
    }
    return '';
  }
  var full = !!button.full;
  var btn = '<a href="' + esc(button.href) + '" style="' +
  ...
  if (secondary && secondary.href) {
    return '<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin-top:28px;"><tr>' +
      ...
      '<td><a href="' + esc(secondary.href) + '" style="font-size:14px; font-weight:700; color:' + C.teal + '; text-decoration:none;">' + esc(secondary.label || 'Mehr') + '</a></td>' +
      '</tr></table>';
  }
...
function htmlNote(note) {
  if (!note) return '';
  return '<p style="margin:22px 0 0; font-size:13px; line-height:1.6; color:' + C.faint + '; font-weight:500;">' + esc(note) + '</p>';
}
...
function htmlPromo() {
  ...
  return '<tr><td style="padding:8px 40px 32px;">' +
    '<div style="background:' + C.teal + '; border-radius:16px; padding:28px 28px 26px; text-align:center;">' +
      ...
      '</tr></table>' +
      '<a href="' + WERBEN_URL + '" style="margin-top:22px; display:block; background:' + C.promoAccent + '; color:' + C.ink + '; font-size:15px; font-weight:800; padding:14px 22px; border-radius:11px; text-decoration:none; letter-spacing:.2px;">Jetzt Freund werben &rarr;</a>' +
    '</div></td></tr>';
}
...
function htmlFooter(kind) {
  ...
  return '<tr><td style="padding:28px 40px 30px; border-top:1px solid ' + C.hair + ';">' +
    '<div style="font-size:13px; font-weight:800; color:' + C.teal + '; letter-spacing:-.2px;">' + esc(STUDIO.name) + '</div>' +
    '<p style="margin:8px 0 0; font-size:12px; line-height:1.7; color:' + C.faint + '; font-weight:500;">' + esc(STUDIO.address) + '<br />' + esc(STUDIO.email) + '</p>' +
    '<div style="margin-top:14px;">' +
      '<a href="' + STUDIO.instagram + '" style="font-size:12px; color:' + C.muted + '; text-decoration:none; font-weight:600; margin-right:18px;">Instagram</a>' +
      '<a href="' + STUDIO.website + '" style="font-size:12px; color:' + C.muted + '; text-decoration:none; font-weight:600;">Website</a>' +
    '</div>' +
    '<p style="margin:16px 0 0; font-size:11px; line-height:1.6; color:' + C.fine + ';">' + fine + '</p>' +
    '</td></tr>';
}
 ...
 *   promo(bool, default false), footer('member'|'security'), preheader(string)
...
function renderEmail(o) {
  ...
  var card = '' +
    '<table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px; max-width:600px; background:#fff; border-radius:16px; overflow:hidden; box-shadow:0 1px 3px rgba(0,0,0,.08);">' +
      htmlHeader() +
      '<tr><td style="padding:44px 40px 36px;">' + bodyInner + '</td></tr>' +
      (o.promo ? htmlPromo() : '') +
  ...
  var html = '<!DOCTYPE html><html lang="de"><head>' +
    ...
    '</head>' +
    '<body style="margin:0; padding:0; background:' + C.page + '; font-family:\'Archivo\', \'Segoe UI\', system-ui, -apple-system, Helvetica, Arial, sans-serif;">' +
    htmlPreheader(o.preheader) +
    '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%; background:' + C.page + ';"><tr>' +
      '<td align="center" style="padding:32px 16px;">' + card + '</td>' +
    '</tr></table>' +
    '</body></html>';
