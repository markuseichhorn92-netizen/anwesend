...
function parseStaff(key, raw, fallbackName) {
  if (!raw) return defaultStaff(key, fallbackName);
  try {
  ...
  } catch (err) { return defaultStaff(key, fallbackName); }
}
...
async function getStaff(id, fallbackName) {
  ...
  return parseStaff(key, s, fallbackName);
}

// Mehrere auf einmal – eine Runde statt einer je Person.
async function staffByIds(ids, namen) {
  const liste = (Array.isArray(ids) ? ids : []).map((x) => String(x == null ? '' : x).trim()).filter(Boolean);
  if (!liste.length) return [];
  if (!hasStore) return liste.map((id) => defaultStaff(id, namen && namen[id]));
  let res;
  try { res = await redisPipeline(liste.map((id) => ['GET', empKey(id)])); }
  catch (e) { return liste.map((id) => defaultStaff(id, namen && namen[id])); }
  return liste.map((id, i) => parseStaff(id, (res || [])[i], namen && namen[id]));
}
...
async function allStaff() {
  ...
  if (!Array.isArray(ids) || !ids.length) return [];
  const out = await staffByIds(ids);
...
module.exports = {
  ...
  getStaff, staffByIds, setStaff, removeStaff, allStaff, monthHours,
