...
async function applyOwnerReply(memberId, vorgangId, rawText, opts) {
  ...
  async function viaWhatsApp() {
    ...
    // 0) Anklickbare Auswahl (Quick-Reply-Buttons) – nur wenn der Aufrufer Optionen
    //    mitgibt und der Anbieter das kann (Meta immer, Twilio nur mit Vorlage). Der
    //    `text` enthält dieselben Optionen bereits nummeriert, daher ist der Fallback
    //    unten (sendText) inhaltlich vollständig, falls der Button-Versand nicht geht.
    if (opts.buttons && Array.isArray(opts.buttons.options) && opts.buttons.options.length >= 2 && typeof WA.sendButtons === 'function') {
      try { const r = await WA.sendButtons(phone, { body: opts.buttons.body || text, options: opts.buttons.options }); if (r && r.ok !== false) return { id: r.id || null }; } catch (e) {}
    }
