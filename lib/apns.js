...
// Den .p8-Schlüssel robust zu sauberem PEM normalisieren. Egal wie er in der
// Env landet (eine Zeile, literal "\n", echte Umbrüche oder "\r\n"): Header/Body
// werden extrahiert, der Base64-Teil von ALLEM Whitespace befreit und mit echten
// 64er-Zeilen neu umbrochen. Das verhindert den OpenSSL-3-Fehler
// „DECODER routines::unsupported" bei zerschossenen Zeilenumbrüchen.
function normalizeP8(raw) {
  ...
  body = body.replace(/\s+/g, '');                 // alle Leerzeichen/Umbrüche raus
  const wrapped = body.match(/.{1,64}/g);
  if (!wrapped) return s;
  return '-----BEGIN ' + header + '-----\n' + wrapped.join('\n') + '\n-----END ' + header + '-----\n';
}

const P8 = normalizeP8(process.env.APNS_PRIVATE_KEY || '');
...
function sendOne(token, msg) {
  return new Promise(function (resolve) {
    if (!hasApns) return resolve({ ok: false, status: 0, error: 'no_apns' });
    // JWT vorab signieren – so unterscheiden wir Signatur- von Verbindungsfehlern.
    let jwt;
    try { jwt = authToken(); } catch (e) { return resolve({ ok: false, status: 0, error: 'jwt:' + ((e && (e.message || e.code)) || 'fail') }); }
    let client;
    try { client = http2.connect(HOST); } catch (e) { return resolve({ ok: false, status: 0, error: 'connect:' + ((e && (e.message || e.code)) || 'fail') }); }
    let done = false;
    const finish = function (r) { if (done) return; done = true; try { client.close(); } catch (e) {} resolve(r); };
    client.on('error', function (e) { finish({ ok: false, status: 0, error: 'conn:' + ((e && (e.code || e.message)) || 'err') }); });
    ...
    } catch (e) { return finish({ ok: false, status: 0, error: 'request:' + ((e && (e.message || e.code)) || 'fail') }); }
    let status = 0, body = '';
    ...
    req.on('error', function (e) { finish({ ok: false, status: 0, error: 'stream:' + ((e && (e.code || e.message)) || 'err') }); });
    req.setTimeout(8000, function () { try { req.close(); } catch (e) {} finish({ ok: false, status: 0, error: 'timeout' }); });
    req.end(payload);
  });
}
