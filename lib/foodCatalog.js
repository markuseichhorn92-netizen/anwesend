...
const CFG = {
  ...
  productLimitPerMin: parseInt(process.env.OPENFOODFACTS_PRODUCT_LIMIT_PER_MINUTE || '', 10) || 60,   // OFF erlaubt 100/min – wir bleiben darunter
...
// Viele Produkte auf einmal schreiben (1 MGET + 1 Pipeline-SET) – für Warmup/Import,
// damit große Batches im Serverless-Timeout bleiben. Idempotent per Barcode.
async function catalogPutMany(prods) {
  if (!hasStore) return 0;
  const list = (prods || []).filter(function (p) { return p && p.barcode; });
  if (!list.length) return 0;
  const now = Date.now();
  const prevMap = {};
  try {
    const keys = list.map(function (p) { return K.product(p.barcode); });
    const [vals] = await redisPipeline([['MGET'].concat(keys)]);
    (vals || []).forEach(function (v, i) { if (v) { try { prevMap[list[i].barcode] = JSON.parse(v); } catch (e) {} } });
  } catch (e) {}
  const cmds = list.map(function (p) {
    const prev = prevMap[p.barcode];
    const rec = Object.assign({}, p, {
      cachedAt: (prev && prev.cachedAt) || p.cachedAt || now,
      lastCheckedAt: now,
      lastUsedAt: (prev && prev.lastUsedAt) || p.lastUsedAt || 0,
      usageCount: (prev && prev.usageCount) || p.usageCount || 0,
      schemaVersion: p.schemaVersion || 1,
    });
    return ['SET', K.product(p.barcode), JSON.stringify(rec)];
  });
  try { await redisPipeline(cmds); } catch (e) { return 0; }
  return list.length;
}
...
module.exports = {
  ...
  catalogGet, catalogPut, catalogPutMany, bumpUsage, negGet, negPut,
