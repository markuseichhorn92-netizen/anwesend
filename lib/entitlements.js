 ...
 * -------------------------------------------------------------------------
 * Ein Datensatz pro Mitglied in Upstash, gesetzt/entzogen über den
 * Magicline-Zusatzmodul-Abgleich (lib/mlPremium) bzw. das Team-Backend
 * (kostenlose Freischaltung). Gelesen von api/member/nutrition.js
 * (Gate + buildState) und api/member/modules.js.
 *
 * Key:   nutri:prem:<memberId>   (memberId = Magicline-Customer-id = sess.id)
 * Wert:  { tier:'premium', status, until, since, source?, moduleContractId?, updatedAt }
 *        status ∈ trialing | active | canceled
 *        until  = Ende der aktuellen Periode (ms) – nach Ablauf kein Premium mehr.
 *        source = 'magicline' (Zusatzmodul, SEPA über den Mitgliedsvertrag,
 *                 siehe lib/mlPremium) | 'team*' (vom Studio kostenlos freigeschaltet).
 *
 ...
 * Kündigung läuft über die Magicline-Modul-Kündigung (siehe api/member/modules.js).
 */
...
// Ohne Abo-Modell (FEATURE_ABO nicht gesetzt) ist ALLES inklusive – für jedes
// Mitglied, unabhängig vom Datensatz. Das ist der eine Hebel, an dem alle
// Premium-Sperren hängen: kein Endpunkt muss dafür angefasst werden.
function aboOn() { return require('./features').aboOn(); }
const INKLUSIVE = Object.freeze({
  premium: true, tier: 'premium', status: 'inklusive',
  everPremium: true, trialEligible: false, trialing: false, trialEnd: null,
  until: null, cancelAtPeriodEnd: false, comp: true, permanent: true,
  source: 'inklusive', viaModule: false, moduleContractId: null,
});

// Premium aktiv? tier=premium UND Status aktiv/Trial UND Periode nicht abgelaufen.
function isPremium(ent) {
  if (!aboOn()) return true;
...
// Client-sicherer Auszug (keine internen IDs an den Browser).
function publicTier(ent) {
  if (!aboOn()) return Object.assign({}, INKLUSIVE);
  const premium = isPremium(ent);
  // Quelle bestimmen: Magicline-Modul oder Team-Comp (kostenlose Freischaltung).
  ...
  return {
    ...
    // Vorgeschichte für adaptive Upgrade-Copy: everPremium = hatte je ein Entitlement
    // (Trial oder bezahlt); trialEligible = noch nie eins -> „X Tage gratis" anbieten.
    everPremium: !!ent,
    trialEligible: !ent,
