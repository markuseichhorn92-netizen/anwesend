...
function publicTier(ent) {
  ...
  // comp = vom Studio (Team) kostenlos freigeschaltet (kein Stripe-Abo) -> keine Zahl-/Kündigen-UI.
  const src = (ent && ent.source) || null;
  const comp = !!(src && String(src).indexOf('team') === 0);
  return {
    premium: premium,
