 ...
 * Rolle einer Sitzung.
 *
 * Fiel frueher auf 'admin' zurueck – ein Uebergangswert fuer Sitzungen aus der
 * Zeit vor dem Rollenfeld. Der hat sich erledigt: Sitzungen laufen nach TTL
 * (12 h) ab, und es gibt genau zwei Wege, eine zu erzeugen (api/team/login),
 * beide setzen die Rolle ausdruecklich.
 *
 * Ein fehlendes Feld ist also kein Altbestand mehr, sondern ein Fehler – und
 * dann ist die kleinere Berechtigung die richtige Antwort. Ein Schreibfehler
 * beim Anlegen der Sitzung machte vorher aus einer Angestellten still eine
 * Administratorin.
 */
function roleOf(sess) {
  if (!sess) return null;
  return sess.role || 'trainer';
}
