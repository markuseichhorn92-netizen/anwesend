...
async function findByPhone(num) {
  ...
  const cands = new Map();   // id -> customer (dedupliziert)
  for (const p of phoneVariants(num)) {
    ...
    if (r && r.status === 200 && Array.isArray(r.json)) {
      for (const c of r.json) { const id = c && (c.id != null ? c.id : c.customerId); if (id != null && !cands.has(String(id))) cands.set(String(id), c); }
    }
  }
  if (!cands.size) return null;
  // Exakter Abgleich anhand der Telefonfelder aus dem Suchergebnis.
  let exact = Array.from(cands.values()).filter((c) => customerPhoneSet(c).has(target));
  // Falls die Suchergebnisse keine Telefonfelder enthalten: Vollprofile gekappt +
  // parallel nachladen und exakt filtern.
  if (!exact.length && cands.size <= 20) {
    const fulls = await Promise.all(Array.from(cands.values()).map(async (c) => {
      const id = c.id != null ? c.id : c.customerId;
      try { return await getMember(id); } catch (e) { return null; }
    }));
    exact = fulls.filter((full) => full && customerPhoneSet(full).has(target));
  }
...
module.exports = {
  ...
  findAndValidate, findByEmailDob, findByNumberDob, findByPhone, phoneSearchDebug, hashCode, otpSave, otpGet, otpDel,
