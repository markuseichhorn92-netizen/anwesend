...
// ── TEMP-Diagnose: Struktur eines Objekts (nur Feldnamen, keine Werte) ──
function shapeOf(o) {
  if (Array.isArray(o)) return { type: 'array', len: o.length, firstKeys: (o[0] && typeof o[0] === 'object') ? Object.keys(o[0]) : [] };
  if (o && typeof o === 'object') {
    var keys = Object.keys(o), nested = {};
    keys.forEach(function (k) {
      var v = o[k];
      if (Array.isArray(v)) nested[k] = 'array[' + v.length + ']' + (v[0] && typeof v[0] === 'object' ? ':{' + Object.keys(v[0]).join(',') + '}' : '');
      else if (v && typeof v === 'object') nested[k] = '{' + Object.keys(v).join(',') + '}';
    });
...
// ── TEMP-Diagnose: welcher Pfad/welche Struktur liefert Vertragsdaten? ──
// Gibt nur Status + Form (Feldnamen, keine Werte) zurück; Fehlertext nur bei !=200.
async function diagContracts(id) {
  var eid = encodeURIComponent(id);
  var paths = [
    '/customers/' + eid + '/contracts',
    '/memberships/' + eid + '/self-service/contract-data',
    '/memberships/' + eid + '/membership-switch/configs',
  ];
  var out = [];
  for (var i = 0; i < paths.length; i++) {
    var p = paths[i], r;
    try { r = await ml('GET', p); } catch (e) { out.push({ path: p, error: String(e && e.message) }); continue; }
    var rec = { path: p, status: r.status, shape: shapeOf(r.json) };
    if (r.status !== 200) rec.body = String(r.text || '').slice(0, 160);   // Fehlertext ist PII-frei
    out.push(rec);
  }
  return out;
}
...
module.exports = {
  ...
  getContract, diagContracts, shapeOf,
};
