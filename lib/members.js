...
async function getContract(id) {
  ...
  var active = statusActive || !ended;

  // Widerrufsrecht (14 Tage) – nur bei online/fern abgeschlossenen Verträgen
  // (contractOrigin WEBCLIENT/ONLINE). Frist ab Vertragsabschluss (createdDate).
  var online = /WEB|ONLINE|ECOM|DISTANCE/i.test(String(c.contractOrigin || ''));
  ...
  if (created) {
    ...
    wdEligible = online && !c.reversed && now.getTime() <= wdDeadline.getTime();
  }
...
module.exports = {
  ...
  findAndValidate, findByEmailDob, findByNumberDob, hashCode, otpSave, otpGet, otpDel,
