...
// Server-seitiger Check-in-Verlauf eines Mitglieds (ohne Mitglieder-Session) für
// Team-/Cron-Zwecke wie Churn-Radar & Reaktivierungs-Nudges. Die Magicline-Liste
// kommt jüngste-zuerst; eine Seite (~50) genügt für „letzter Besuch" und einen
// kurzen Frequenztrend. Über opts.pages erweiterbar. 403/Fehler -> []. Ergebnis
// ist absteigend nach Datum sortiert (Index 0 = letzter Check-in).
async function recentCheckins(id, opts) {
  opts = opts || {};
  var maxPages = Math.max(1, opts.pages || 1);
  var SLICE = 50;
  var all = [];
  var offset = 0;
  try {
    for (var page = 0; page < maxPages; page++) {
      var q = '/customers/' + encodeURIComponent(id) + '/activities/checkins?sliceSize=' + SLICE + '&offset=' + offset;
      var r = await ml('GET', q);
      ...
      var next = parseInt(r.json.offset, 10);
      offset = Number.isFinite(next) && next > offset ? next : offset + SLICE;
    }
  } catch (e) {}
  return all.map(function (c) { return { in: c.checkInDateTime || null, out: c.checkOutDateTime || null, studio: c.studioName || null }; })
    .filter(function (c) { return c.in; })
    .sort(function (a, b) { return new Date(b.in) - new Date(a.in); });
}

// Tage seit dem letzten Check-in (ganzzahlig) oder null, wenn keiner vorliegt.
async function daysSinceLastCheckin(id, opts) {
  var cis = await recentCheckins(id, opts);
  if (!cis.length) return null;
  var last = new Date(cis[0].in).getTime();
...
module.exports = {
  ...
  getContract, recentCheckins, daysSinceLastCheckin,
};
