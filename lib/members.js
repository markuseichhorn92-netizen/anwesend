...
// Sitzung serverseitig widerrufen (Logout). Idempotent: fehlende/abgelaufene
// Tokens sind kein Fehler. Der Token wird bewusst nirgends geloggt.
async function destroySession(token) {
  if (!token || !hasStore) return;
  try { await redisPipeline([['DEL', 'msess:' + token]]); } catch (e) {}
}
function bearer(req) {
...
// Welche Magicline-„contractOrigin"-Werte gelten als ECHTER Online-/Fernabsatz-
// Abschluss? NUR solche Verträge haben ein 14-tägiges Widerrufsrecht – im Studio
// unterschriebene Verträge NICHT. Bewusst OHNE das generische „CONNECT_API": dieser
// Integrations-Kanal wird auch für im Studio/Backend angelegte Verträge genutzt und
// ist daher kein verlässliches Online-Signal (führte zu falschem „Widerruf" bei
// normalen Verträgen). Exakt konfigurierbar über die Env CONTRACT_ONLINE_ORIGINS
// (kommagetrennt) – dort die tatsächlichen Online-Origin-Werte des Studios eintragen.
var ONLINE_ORIGIN_KEYS = String(process.env.CONTRACT_ONLINE_ORIGINS
  || 'WEBSITE,WEBSHOP,WEB_SHOP,ONLINESHOP,ONLINE_SHOP,ECOMMERCE,E_COMMERCE,DISTANCE,FERNABSATZ,ONLINE_SALE,ONLINESALE')
  .split(',').map(function (s) { return s.trim().toUpperCase(); }).filter(Boolean);
function isOnlineOrigin(origin) {
  var o = String(origin || '').trim().toUpperCase();
  if (!o) return false;
  return ONLINE_ORIGIN_KEYS.some(function (k) { return k && o.indexOf(k) >= 0; });
}

// Betrag auf Wochenbasis normalisieren (das Studio rechnet wöchentlich ab). Reine, testbare Funktion.
// Unbekannte/leere Einheit -> als Wochenbeitrag interpretiert (Studio-Konvention). 0/ungültig -> null.
function weeklyFromPrice(price, unit) {
  const p = Number(price);
  if (!isFinite(p) || p <= 0) return null;
  const u = String(unit || '').toUpperCase();
  let w;
  if (u === 'WEEK' || u === 'WEEKLY' || u === 'WOCHE') w = p;
  else if (u === 'MONTH' || u === 'MONTHLY' || u === 'MONAT') w = (p * 12) / 52;
  else if (u === 'YEAR' || u === 'YEARLY' || u === 'ANNUAL' || u === 'JAHR') w = p / 52;
  else if (u === 'DAY' || u === 'DAILY' || u === 'TAG') w = p * 7;
  else if (u === 'QUARTER' || u === 'QUARTERLY' || u === 'QUARTAL') w = (p * 4) / 52;
  else if (u === 'HALFYEAR' || u === 'SEMIANNUAL' || u === 'HALBJAHR') w = (p * 2) / 52;
  else w = p;
  return Math.round(w * 100) / 100;
}
// Defensiver Feld-Zugriff: die Preis-/Zahlweise-Felder im Magicline-Vertrag variieren je Tarif/Version.
function pickPath(obj, path) {
  try { return path.split('.').reduce((o, k) => (o == null ? undefined : o[k]), obj); } catch (e) { return undefined; }
}
function firstNum(obj, paths) {
  for (const path of paths) { const v = Number(pickPath(obj, path)); if (isFinite(v) && v > 0) return v; }
  return null;
}
function firstStr(obj, paths) {
  for (const path of paths) { const v = pickPath(obj, path); if (v != null && String(v).trim()) return String(v).trim(); }
  return null;
}
...
async function getContract(id) {
  ...
  // Widerrufsrecht (14 Tage) – NUR bei echt online/im Fernabsatz abgeschlossenen
  // Verträgen (siehe isOnlineOrigin). Im Studio unterschriebene Verträge haben KEIN
  // Widerrufsrecht. Frist ab Vertragsabschluss (createdDate).
  var online = isOnlineOrigin(c.contractOrigin);
  ...
  // Wochenbeitrag best-effort aus dem Magicline-Vertrag ableiten (Feldnamen variieren; nicht
  // gefunden -> null). Das Team kann den Betrag im Backend ohnehin überschreiben.
  const rawPrice = firstNum(c, [
    'price', 'grossPrice', 'amount', 'monthlyPrice', 'paymentAmount', 'contractVolume',
    'rate.price', 'rate.grossPrice', 'rate.amount',
    'priceInformation.grossAmount', 'priceInformation.amount', 'priceInformation.price',
    'contractVolumeInformation.paymentFrequencyAmount', 'contractVolumeInformation.amount',
  ]);
  const freqUnit = firstStr(c, [
    'paymentFrequencyUnit', 'paymentFrequency.unit', 'paymentFrequency',
    'billingFrequencyUnit', 'billingFrequency', 'rate.paymentFrequencyUnit',
    'priceInformation.paymentFrequencyUnit', 'contractVolumeInformation.paymentFrequencyUnit',
  ]);
  const weeklyPrice = weeklyFromPrice(rawPrice, freqUnit);
...
module.exports = {
  ...
  createSession, getSession, destroySession, bearer, rateLimit, readBody,
  ...
  getContract, weeklyFromPrice, recentCheckins, daysSinceLastCheckin, checkinHistory,
  isOnlineOrigin,
};
