...
async function diagContracts(id) {
  var paths = [
    '/customers/' + encodeURIComponent(id) + '/contracts',
    '/contracts?customerId=' + encodeURIComponent(id),
    '/customers/' + encodeURIComponent(id) + '/memberships',
    '/memberships?customerId=' + encodeURIComponent(id),
    '/customers/' + encodeURIComponent(id) + '/contract',
  ];
  var out = [];
  for (var i = 0; i < paths.length; i++) {
    ...
    var j = r.json, shape;
    if (Array.isArray(j)) shape = { type: 'array', len: j.length, firstKeys: j[0] ? Object.keys(j[0]) : [] };
    else if (j && typeof j === 'object') shape = { type: 'object', keys: Object.keys(j) };
    else shape = { type: typeof j };
    var rec = { path: p, status: r.status, shape: shape };
