...
// Probiert ALLE Formatvarianten durch und gibt die ERSTE zurück, die genau einen
// Treffer liefert. Mehrdeutige Varianten werden übersprungen (nicht abgebrochen),
// damit eine spezifischere Variante (z. B. genau das gespeicherte 015120442244)
// noch zum Zug kommt. So findet die Zuordnung die Nummer unabhängig vom Format.
async function findByPhone(num) {
  for (const p of phoneVariants(num)) {
    ...
    if (r && r.status === 200 && Array.isArray(r.json) && r.json.length === 1) {
      return r.json[0];
    }
  }
  return null;   // kein eindeutiger Treffer
}
