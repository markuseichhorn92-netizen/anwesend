...
function sanitizeSession(o, tsNow) {
  ...
  const sess = {
    ...
    note: str(o.note, 140),
    // Herkunft: in der App aufgezeichnet ('app') oder aus Apple Health / Google Fit / Uhr importiert.
    source: (['app', 'health', 'watch', 'manual'].indexOf(o.source) >= 0) ? o.source : 'app',
  };
  // Externe Kennung (Health/Fit) – dedupliziert Re-Importe.
  if (o.extId != null && String(o.extId)) sess.extId = str(o.extId, 80);
  if (kind === 'outdoor') {
...
// Gültig, wenn lang genug UND entweder Puls ODER (outdoor) eine Strecke ODER
// (Import aus Health/Fit) eine belastbare Zusammenfassung (kcal bzw. ≥1 Min) vorliegt.
function validSession(s) {
  if (!s || s.durationSec < 10) return false;
  if (s.avgHr > 0) return true;
  if (s.kind === 'outdoor' && s.distanceM > 0) return true;
  if (s.source && s.source !== 'app' && (s.kcal > 0 || s.durationSec >= 60)) return true;
  return false;
}

// Datum (YYYY-MM-DD) aus Millisekunden.
function dateFromMs(ms) {
  try { const d = new Date(ms); if (isNaN(d.getTime())) return ''; return d.getFullYear() + '-' + ('0' + (d.getMonth() + 1)).slice(-2) + '-' + ('0' + d.getDate()).slice(-2); } catch (e) { return ''; }
}

// Externe Einheit (Apple Health / Google Fit / Uhr) -> unser Session-Rohformat.
// Erwartet { extId, start(ms), end(ms), durationSec?, activity, kind?, kcal, avgHr, maxHr, distanceM, source? }.
// Die eigentliche Klammerung übernimmt danach sanitizeSession(fromExternal(w), ts).
function fromExternal(w) {
  w = w && typeof w === 'object' ? w : {};
  const start = n(w.start, 1, 9e15) || 0;
  const end = n(w.end, 1, 9e15) || 0;
  let durationSec = n(w.durationSec, 1, 86400) || 0;
  if (!durationSec && start && end && end > start) durationSec = Math.round((end - start) / 1000);
  const activity = ACTIVITIES[w.activity] ? w.activity : (OUTDOOR[w.activity] ? w.activity : 'outdoor');
  const kind = (w.kind === 'indoor') ? 'indoor' : (OUTDOOR[activity] ? 'outdoor' : 'indoor');
  const source = (w.source === 'watch' || w.source === 'manual') ? w.source : 'health';
  const out = {
    ts: start || (n(w.ts, 1, 9e15) || 0),
    date: /^\d{4}-\d{2}-\d{2}$/.test(String(w.date || '')) ? String(w.date) : dateFromMs(start),
    kind, activity, source, extId: w.extId,
    durationSec, avgHr: w.avgHr, maxHr: w.maxHr, kcal: w.kcal,
  };
  if (kind === 'outdoor') out.distanceM = w.distanceM;
  return out;
}

// Import von externen Einheiten: säubern, gegen vorhandene (extId) entdoppeln, einmischen.
// Rein (ohne Store) und damit unit-testbar. -> { list (sortiert, gekappt), added }.
function importMerge(existing, incoming, tsNow) {
  const cur = Array.isArray(existing) ? existing.slice() : [];
  const seen = {};
  cur.forEach((s) => { if (s && s.extId) seen[String(s.extId)] = 1; });
  let added = 0;
  (Array.isArray(incoming) ? incoming : []).forEach((raw) => {
    const ext = fromExternal(raw);
    const s = sanitizeSession(ext, ext.ts || tsNow || 0);
    if (!validSession(s)) return;
    if (s.extId && seen[String(s.extId)]) return;   // schon importiert
    if (s.extId) seen[String(s.extId)] = 1;
    cur.push(s); added++;
  });
  cur.sort((a, b) => (b.ts || 0) - (a.ts || 0));
  return { list: cur.slice(0, CAP), added };
}
...
// Externe Einheiten (Health/Fit) übernehmen: dedupliziert per extId, schreibt gemischt zurück.
async function importMany(id, incoming, tsNow) {
  if (!hasStore || id == null) return { ok: false, added: 0, list: [] };
  try {
    const arr = await list(id);
    const merged = importMerge(arr, incoming, tsNow);
    if (merged.added > 0) await redisPipeline([['SET', KEY(id), JSON.stringify(merged.list), 'EX', String(TTL)]]);
    return { ok: true, added: merged.added, list: merged.list };
  } catch (e) { return { ok: false, added: 0, list: await list(id) }; }
}
...
module.exports = {
  ...
  fromExternal, importMerge, dateFromMs,
  list, add, remove, clear, importMany,
};
