...
async function stripe(method, path, params) {
  ...
  const opt = { method: method, headers: { Authorization: 'Bearer ' + SECRET, Accept: 'application/json' } };
...
// Wiederkehrenden Stripe-Kunden anlegen (nur wenn noch keiner existiert).
async function createCustomer(opts) {
  opts = opts || {};
  const r = await stripe('POST', '/customers', {
...
module.exports = {
  hasStripe, BASE, PRICE_ID, TRIAL_DAYS, WEBHOOK_SECRET,
  toForm, stripe, createCustomer, createCheckoutSession, createPortalSession,
