...
// Biostrength-Übungen: KEINE Wiederholungen/Gewichte vorgeben – das steuert die
// Technogym-KI. Solche Übungen werden nur „abgehakt". (bio-Flag setzen, Werte entfernen.)
PLANS.forEach(function (p) {
  p.days.forEach(function (d) {
    d.exercises.forEach(function (e) {
      if (/biostrength/i.test(e.machine || '')) { e.bio = true; delete e.sets; delete e.reps; delete e.rest; }
    });
  });
});
...
function normExercise(x) {
  ...
  if (!name) return null;
  const machine = str(x.machine || x.geraet || x.equipment, 60);
  const bio = !!x.bio || /biostrength/i.test(machine);
  const e = { name: name };
  if (machine) e.machine = machine;
  if (bio) { e.bio = true; }   // Biostrength: Technogym-KI steuert Gewicht/Wiederholungen – keine Vorgabe, nur abhaken
  else {
    e.sets = clampN(x.sets, 1, 12, 3);
    e.reps = str(x.reps || x.wiederholungen || x.reps_text, 30) || '10–12';
    e.rest = str(x.rest || x.pause, 20) || '60 s';
  }
...
async function resolveActive(id) {
  ...
  if ((a.source === 'finn' || a.source === 'team') && a.plan) {
    return { plan: Object.assign({}, a.plan, { source: a.source }), startedAt: a.startedAt || null, assignedByTeam: a.assignedByTeam || null };
  }
  return null;
}
