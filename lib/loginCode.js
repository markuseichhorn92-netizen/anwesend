...
async function noteDelivery(messageId, status) {
  ...
  messLog('delivery', { via: rec.via, status: String(status || ''), sekunden: sek, api_ms: rec.api });
  return sek;
}
...
async function tryWhatsApp(member, opts, code) {
  ...
  if (!WA.hasWaLogin) { messLog('handover', { via: 'whatsapp', ms: 0, ok: false, grund: 'keine Vorlage konfiguriert' }); return false; }
  const phone = await resolvePhone(member, opts);
  if (!phone) { messLog('handover', { via: 'whatsapp', ms: 0, ok: false, grund: 'keine Rufnummer am Konto' }); return false; }
  const t0 = Date.now();
  try {
...
module.exports = { sendLoginCode, noteDelivery };
