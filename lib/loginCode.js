...
async function messUebergabe(kanal, ms, ok, id) {
  messLog('handover', { via: kanal, ms: ms, ok: !!ok });
...
async function tryWhatsApp(member, opts, code) {
  ...
  try {
    ...
    const ok = !!(r && r.ok);
    await messUebergabe('whatsapp', Date.now() - t0, ok, r && r.id);
    return ok;
  } catch (e) {
    await messUebergabe('whatsapp', Date.now() - t0, false, null);
    return false;
  }
}
