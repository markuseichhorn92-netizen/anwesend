...
async function sendLoginCode(member, host, opts) {
  ...
  if (opts.channel === 'whatsapp' && WA.hasWaLogin) {
    // Nummer: explizit (über Dubletten ermittelt) ODER aus dem Login-/Suchobjekt ODER –
    // da die Magicline-Suche oft KEINE Telefonfelder liefert – aus dem Vollprofil nachladen.
    let phone = opts.phone || member.phonePrivate || member.phoneMobile || member.phoneBusiness || null;
