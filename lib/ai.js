...
async function nutritionRecipes(ctx) {
  ctx = ctx || {};
  if (!hasAI) return { ok: false, error: 'no_ai_key' };
  // Mahlzeiten-Fragebogen: pro genannter Mahlzeit ({label,kcal,protein}) genau ein Rezept.
  const meals = (Array.isArray(ctx.meals) ? ctx.meals : []).filter(function (m) { return m && m.label; }).slice(0, 4);
  ...
  const sys = [
    ...
    (meals.length ? '- Erstelle GENAU ein Rezept pro unten genannter Mahlzeit, in exakt der genannten Reihenfolge. Jedes Rezept bekommt das Feld "meal" mit dem Namen der Mahlzeit (exakt wie genannt).' : ''),
    ...
    (meals.length ? '- WICHTIG: Stimme jedes Rezept auf DIESE Person ab. EINE Portion soll ungefähr die für seine Mahlzeit genannte kcal-/Eiweiß-Vorgabe treffen. Passe Portionsgröße, Zutaten und Mengen (Gramm) entsprechend an.'
      : (mealKcal ? ('- WICHTIG: Stimme jedes Rezept auf DIESE Person ab. EINE Portion soll ungefähr ' + mealKcal + ' kcal' + (mealProtein ? (' und ' + mealProtein + ' g Eiweiß') : '') + ' liefern. Passe Portionsgröße, Zutaten und Mengen (Gramm) so an, dass die Werte pro Portion dieses Ziel treffen.') : '- Halte die Portionen alltagstauglich und passend zum Ziel.')),
  ...
  const user = 'Ziel: ' + (ctx.goal || 'ausgewogen ernähren')
    ...
    + (meals.length ? (' Plane passende Rezepte für diese Mahlzeiten (Reihenfolge exakt einhalten): ' + meals.map(function (m) { return String(m.label) + ' (~' + (parseInt(m.kcal, 10) || 0) + ' kcal' + (parseInt(m.protein, 10) ? (', ~' + (parseInt(m.protein, 10)) + ' g Eiweiß') : '') + ')'; }).join('; ') + '.')
...
async function nutritionMealReview(ctx) {
  ...
  const r = await complete(sys, [{ role: 'user', content: user }], { maxTokens: 500, temperature: 0.4 });
  if (!r.ok) return r;
  const j = parseJsonLoose(r.answer);
  if (!j) return { ok: false, error: 'parse_failed' };
...
module.exports = { askHelp, coachReply, coachTip, draftReply, estimateFood, estimateFoodPhoto, scanAttest, conciergeReply, nutritionRecipes, nutritionReply, nutritionWeekPlan, nutritionWeekReview, nutritionMealReview, coachLessonPersonalize, coachDailyImpulse, coachCheckinReview, messagesRaw, winbackSuggest, winbackMessage, hasAI, MODEL };
