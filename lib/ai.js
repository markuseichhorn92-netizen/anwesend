...
async function requestMessages(payload, signal, errorLimit, securityScope) {
  ...
  try {
    ...
    if (canConverse) {
      ...
      if (HAS_BEDROCK_GUARDRAIL) input.guardrailConfig = {
    ...
    if (HAS_BEDROCK_GUARDRAIL) {
      invokeInput.guardrailIdentifier = BEDROCK_GUARDRAIL_ID;
      invokeInput.guardrailVersion = BEDROCK_GUARDRAIL_VERSION;
    }
