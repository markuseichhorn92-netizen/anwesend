...
async function coachMorningCheck(ctx) {
  ...
  if (!hasAI) return { ok: false, error: 'no_ai_key' };
  const sys = [
    'Du bist FINN, Coach von Fit-Inn Trier (Fitnessstudio). Ein Mitglied hat morgens mit dem Polar-H9-Brustgurt gemessen. Werte die Trainingsbereitschaft für heute aus – auf Basis von Ruhepuls (und, falls vorhanden, HRV) im Vergleich zur PERSÖNLICHEN Baseline. Per "du", Deutsch, warmherzig und konkret.',
