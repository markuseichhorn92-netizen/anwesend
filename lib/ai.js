...
async function requestMessages(payload, signal, errorLimit) {
  errorLimit = errorLimit || 300;
  const assessment = AISecurity.assessMessages(payload && payload.messages);
  if (!assessment.ok) {
    return { ok: false, status: 400, error: 'prompt_attack_blocked', securityReason: assessment.reason };
  }
  payload = AISecurity.hardenPayload(payload || {});
  if (!IS_BEDROCK) {
...
async function messagesRaw(opts) {
  ...
  try {
    ...
    const to = aiTimeout();
    const r = await requestMessages(payload, to.signal, 300);
    to.done();
