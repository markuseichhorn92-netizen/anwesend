...
async function scanMetabolic(base64, mediaType, opts) {
  ...
  const sys = [
    'Du wertest die Stoffwechselanalyse (z. B. Atemgasanalyse/indirekte Kalorimetrie) eines',
    'Fitnessstudio-Mitglieds aus und schlägst dem TRAINER einen Ernährungs-Phasenplan vor.',
    'Der Vorschlag wird vom Menschen geprüft, bevor er gilt.',
    '',
    ...
    'Gesucht: Grundumsatz/Ruheumsatz (bmr bzw. gemessen rmrMeasured, kcal), Gesamtumsatz/',
    'Leistungsumsatz (tdee, kcal), respiratorischer Quotient (rq), Körperfettanteil (fatPct, %),',
    'Gewicht (kg), empfohlene Kalorien (kcalRecommended), Messdatum (date, YYYY-MM-DD), Gerät (device).',
    '',
    'AUFGABE 2 – Phasenplan vorschlagen (2 bis 4 Phasen, zusammen etwa ' + weeks + ' Wochen).',
  ...
  const num = function (v, lo, hi, dec) { const x = Number(v); if (!isFinite(x)) return null; const f = Math.pow(10, dec == null ? 1 : dec); return Math.max(lo, Math.min(hi, Math.round(x * f) / f)); };
  ...
  return {
    ok: true, analysis: analysis, phases: phases,
