...
function kbCollectObjects(answer) {
  ...
  // 1) Array/Objekt aus roher UND normalisierter Fassung schneiden (erste, die trägt, gewinnt).
  for (const src of [raw, norm(raw)]) {
  ...
  if (!out.length) {
    (norm(raw).match(/\{[^{}]*\}/g) || []).forEach((chunk) => { const v = tryParse(chunk); if (v && typeof v === 'object') out.push(v); });
  }
  return out;
}
