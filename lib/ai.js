...
async function trainingPlan(ctx) {
  ...
  const sys = [
    ...
    'Antworte AUSSCHLIESSLICH mit JSON, ohne Text drumherum, in genau dieser Form:',
    '{"title":"…","subtitle":"…","summary":"1-2 Sätze","weeks":8,"focus":["…"],"days":[{"name":"z. B. Ganzkörper A","subtitle":"optional","exercises":[{"name":"Übung","sets":3,"reps":"10–12","rest":"75 s","note":"optional kurzer Hinweis"}]}],"tips":["…"]}',
  ...
  const outDays = rawDays.slice(0, 6).map(function (d) {
    ...
    const ex = (Array.isArray(d.exercises) ? d.exercises : []).slice(0, 8).map(function (e) {
      ...
      return { name: name, sets: s, reps: String(e.reps || '10–12').slice(0, 30), rest: String(e.rest || '60 s').slice(0, 20), note: e.note ? String(e.note).slice(0, 140) : undefined };
    }).filter(Boolean);
