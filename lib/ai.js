...
async function trainingPlan(ctx) {
  ...
  const outDays = rawDays.slice(0, 6).map(function (d) {
    ...
    const ex = (Array.isArray(d.exercises) ? d.exercises : []).slice(0, 8).map(function (e) {
      ...
      return { name: name, machine: e.machine ? String(e.machine).slice(0, 60) : undefined, sets: s, reps: String(e.reps || '10–12').slice(0, 30), rest: String(e.rest || '60 s').slice(0, 20), note: e.note ? String(e.note).slice(0, 140) : undefined };
    }).filter(Boolean);
