...
async function requestMessages(payload, signal, errorLimit) {
  ...
  try {
    ...
    delete body.model;
    const out = await getBedrockClient().send(new Bedrock.InvokeModelCommand({
      modelId: payload.model,
    ...
    const errorMessage = String((e && e.message) || '')
      .replace(/\s+/g, ' ')
      .slice(0, 500);
    // Intentionally log no prompt, response body, member data or credentials.
    console.error('[ai-bedrock]', JSON.stringify({ name: errorName, status: status, message: errorMessage }));
    return {
      ok: false,
      status: status,
