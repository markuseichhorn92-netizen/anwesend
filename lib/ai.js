...
// ctx = { goal, kcalTarget, protein, diet, wish, count }
...
async function nutritionRecipes(ctx) {
  ctx = ctx || {};
  if (!hasAI) return { ok: false, error: 'no_ai_key' };
  const count = Math.max(1, Math.min(3, parseInt(ctx.count, 10) || 3));
  const sys = [
  ...
  const r = await complete(sys, [{ role: 'user', content: user }], { maxTokens: 1500, temperature: 0.6 });
  if (!r.ok) return r;
  const j = parseJsonLoose(r.answer);
