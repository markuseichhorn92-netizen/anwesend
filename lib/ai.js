...
const MODEL_PLAN = IS_BEDROCK ? (process.env.BEDROCK_MODEL_PLAN_ID || MODEL) : (process.env.AI_MODEL_PLAN || MODEL);
const MODEL_ANALYSIS = IS_BEDROCK ? (process.env.BEDROCK_MODEL_ANALYSIS_ID || MODEL) : (process.env.AI_MODEL_ANALYSIS || MODEL);
const hasAI = IS_BEDROCK ? Boolean(BEDROCK_ROLE_ARN && BEDROCK_MODEL && BEDROCK_REGION === 'eu-central-1') : Boolean(KEY);
...
async function requestMessages(payload, signal, errorLimit) {
  ...
  try {
    const Bedrock = require('@aws-sdk/client-bedrock-runtime');
    const body = Object.assign({ anthropic_version: 'bedrock-2023-05-31' }, payload);
    delete body.model;
    ...
    return {
      ok: false,
      status: status,
      error: String((e && (e.name || e.message)) || 'bedrock_error').slice(0, errorLimit),
    };
  }
}
