...
function cacheBlocks(stable, dynamic) {
  const blocks = [{ type: 'text', text: String(stable || ''), cache_control: { type: 'ephemeral' } }];
...
function coachSystemParts(member, articles) {
  ...
  const dynamic = [today, ctx, details].filter(function (x) { return x; }).join('\n');
...
async function trainingPlan(ctx) {
  ...
  const sys = [
    ...
    'Antworte AUSSCHLIESSLICH mit JSON, ohne Text drumherum, in genau dieser Form:',
    '{"title":"…","subtitle":"…","summary":"1-2 Sätze","weeks":8,"focus":["…"],"days":[{"name":"z. B. Ganzkörper A","subtitle":"optional","exercises":[{"name":"Übung","machine":"passendes Gerät im Studio (optional)","sets":3,"reps":"10–12","rest":"75 s","note":"optional kurzer Hinweis"}]}],"tips":["…"]}',
  ...
  const outDays = rawDays.slice(0, 6).map(function (d) {
    ...
    const ex = (Array.isArray(d.exercises) ? d.exercises : []).slice(0, 8).map(function (e) {
      ...
      if (bio) return { name: name, machine: machine, bio: true, note: e.note ? String(e.note).slice(0, 140) : undefined };
      let s = parseInt(e.sets, 10); if (isNaN(s) || s < 1) s = 3; if (s > 8) s = 8;
      return { name: name, machine: machine, sets: s, reps: String(e.reps || '10–12').slice(0, 30), rest: String(e.rest || '60 s').slice(0, 20), note: e.note ? String(e.note).slice(0, 140) : undefined };
    }).filter(Boolean);
...
module.exports = { askHelp, coachReply, coachTip, coachInsight, draftReply, estimateFood, estimateFoodPhoto, scanAttest, conciergeReply, nutritionRecipes, nutritionReply, nutritionWeekPlan, trainingPlan, nutritionWeekReview, nutritionMealReview, coachLessonPersonalize, coachDailyImpulse, coachCheckinReview, figurReview, messagesRaw, winbackSuggest, winbackMessage, cacheBlocks, hasAI, MODEL };
