...
async function coachTip(statsLine) {
  ...
  const user = 'Stand des Mitglieds: ' + (statsLine || 'aktiv') + '. Gib einen kleinen, konkreten Impuls für heute.';
  return complete(sys, [{ role: 'user', content: user }], { maxTokens: 120, temperature: 0.85 });
}

module.exports = { askHelp, coachReply, coachTip, hasAI, MODEL };
