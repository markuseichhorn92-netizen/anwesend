...
// RMSSD-Ausreißer entschärfen: Kurzzeit-RMSSD liegt physiologisch meist ~15–110 ms.
// Ein einzelner, stark abweichender Wert (z. B. ein als RMSSD fehl-importierter SDNN-
// Wert ~120–160 ms, oder ein Messfehler) darf Baseline/Referenz nicht verzerren – sonst
// sieht eine völlig normale Messung „unter Baseline" aus und löst einen Fehlalarm aus.
// Deshalb winsorizen wir gegen den Median (Werte in einen plausiblen Korridor ziehen).
function winsorizeHrv(vals) {
  const arr = (vals || []).filter((v) => v != null);
  if (arr.length < 3) return arr.slice();
  const med = median(arr);
  if (med == null || med <= 0) return arr.slice();
  const hi = med * 2, lo = med * 0.4;
...
function baseline(list) {
  ...
  const hrvVals = winsorizeHrv(prior.map((m) => m.hrvRmssd).filter((v) => v != null));
...
function overtraining(list) {
  ...
  const hrvVals = winsorizeHrv(ref.map((m) => m.hrvRmssd).filter((v) => v != null));
  // Stabile Haupt-Baseline (Median über bis zu BASE_WINDOW ältere Tage) als Anker.
  const main = baseline(arr);
  let baseRhr = rhrVals.length >= 2 ? median(rhrVals) : main.rhr;
  let baseHrv = hrvVals.length >= 2 ? median(hrvVals) : main.hrv;
  // Ausreißer-Schutz: eine schmale 2–3-Tage-Referenz kann durch einen einzelnen
  // Fehlwert (z. B. fehl-importierter SDNN) massiv verzerrt sein. Weicht sie stark von
  // der stabilen Haupt-Baseline ab, gewinnt die Haupt-Baseline – sonst entsteht aus
  // einer top Messung ein Fehlalarm „Erholung unter Baseline".
  if (main.hrv != null && baseHrv != null && (baseHrv > main.hrv * 1.4 || baseHrv < main.hrv * 0.6)) baseHrv = main.hrv;
  if (main.rhr != null && baseRhr != null && Math.abs(baseRhr - main.rhr) > 8) baseRhr = main.rhr;
...
// EIN einziges, widerspruchsfreies Tages-Urteil. Fasst Physiologie (readiness),
// Frühwarnung (overtraining), aktive Erholungsphase (recovery) und die subjektiven
// Signale (trainingLoad-Cap) zu GENAU EINER Empfehlung zusammen – damit nicht mehrere
// Karten gleichzeitig „Vollgas" und „Ruhetag" sagen. Reine Funktion.
// opts: { readiness, overtraining, recovery, latest }
// -> { level:'gruen'|'gelb'|'rot'|'kalibrierung', intensity:'voll'|'moderat'|'ruhe'|'kalibrierung',
//      headline, detail, source, recoveryStale }
function dayVerdict(opts) {
  opts = opts || {};
  const r = opts.readiness || {};
  const ot = opts.overtraining || {};
  const rec = (opts.recovery && opts.recovery.active) ? opts.recovery : null;
  const latest = opts.latest || null;
  const physioLevel = (r.level && r.level !== 'kalibrierung') ? r.level : null;   // gruen|gelb|rot|null
  const alert = ot.level === 'alert';
  const sf = subjectiveFlags(latest);        // { any, notes, cap }
  const sick = !!(latest && latest.sick === 1);
  // „Stale" Erholungsphase: heute wieder klar erholt UND keine Frühwarnung mehr -> die
  // laufende Pause passt nicht mehr zum aktuellen Zustand (Vorschlag: beenden).
  const recoveryStale = !!(rec && physioLevel === 'gruen' && !alert);

  if (!physioLevel && !alert && !rec) {
    return { level: 'kalibrierung', intensity: 'kalibrierung', headline: 'Kalibrierung läuft', detail: 'Miss ein paar Tage – dann gibt es eine klare Tages-Empfehlung.', note: '', source: 'kalibrierung', recoveryStale: false };
  }
  // Aktive Erholungsphase gewinnt – AUSSER sie ist „stale" (heute wieder grün): dann kein
  // harter Ruhetag mehr, sondern der echte grüne Zustand + Hinweis, die Pause zu beenden.
  if (rec && !recoveryStale) {
    return { level: 'rot', intensity: 'ruhe', headline: 'Erholungsphase aktiv', detail: 'Du bist in einer bewussten Erholungsphase – leichte Bewegung, gutes Essen und Schlaf sind jetzt dein Training.', note: '', source: 'recovery', recoveryStale: false };
  }
  // Echte Übertrainings-Warnung -> Ruhe (nach dem Ausreißer-Schutz kein Fehlalarm mehr).
  if (alert) {
    return { level: 'rot', intensity: 'ruhe', headline: 'Erholung hat Vorrang', detail: ot.note || 'Deine Erholung liegt seit mehreren Messungen unter deiner Baseline – heute regenerieren.', note: '', source: 'alert', recoveryStale: recoveryStale };
  }
  // Krankheit ist der EINZIGE subjektive Faktor, der eine gute Physiologie überstimmt
  // (Gesundheit/Sicherheit): angeschlagen -> Ruhetag.
  if (sick) {
    return { level: 'rot', intensity: 'ruhe', headline: 'Heute Ruhetag', detail: 'Du hast dich als angeschlagen gemeldet – gib deinem Immunsystem Vorrang. Sport bei einem Infekt kann das Herz belasten.', note: '', source: 'krank', recoveryStale: recoveryStale };
  }
  // Sonst folgt die Empfehlung der PHYSIOLOGIE (Ruhepuls/HRV vs. Baseline). Weiche
  // subjektive Signale (schlecht geschlafen, Stress, Muskelkater) sind ein HINWEIS, kippen
  // aber die objektiv gute Erholung NICHT nach unten (die HRV bildet die Schlafqualität
  // bereits ab). Sonst entstehen widersprüchliche Karten: grüner Score „100", aber „moderat".
  const MAP = {
    gruen: { intensity: 'voll', headline: 'Bereit für Vollgas', detail: 'Dein System ist erholt – heute darfst du intensiv/hart trainieren.' },
    gelb: { intensity: 'moderat', headline: 'Moderat starten', detail: 'Teilweise erholt – heute locker bis mittel, kein Maximalreiz.' },
    rot: { intensity: 'ruhe', headline: 'Heute auf Erholung setzen', detail: 'Dein Körper steht unter Last – heute Ruhetag, Spaziergang oder lockeres Cardio.' },
  };
  const base = MAP[physioLevel] || MAP.gruen;
  const note = sf.any ? ('Du hast ' + sf.notes.join(', ') + ' gemeldet – achte heute besonders auf dich: gut aufwärmen und auf deinen Körper hören.') : '';
  return { level: physioLevel, intensity: base.intensity, headline: base.headline, detail: base.detail, note: note, source: (sf.any ? 'physio+hinweis' : 'physio'), recoveryStale: recoveryStale };
}
...
function toPromptText(list, opts) {
  ...
  // EIN widerspruchsfreies Fazit (Physiologie + Frühwarnung + Erholungsphase + subjektive
  // Signale bereits gegeneinander verrechnet). FINN bekommt NICHT mehrere konkurrierende
  // Signale, sondern genau diese eine Linie – so kann er keine widersprüchliche Empfehlung
  // ausgeben. Das rohe Übertrainings-Signal wird bewusst NICHT zusätzlich genannt.
  const verdict = dayVerdict({ readiness: r, overtraining: ot, recovery: opts && opts.recovery, latest: m });
  if (verdict && verdict.intensity !== 'kalibrierung') {
    const intoWords = { voll: 'volle Belastung möglich (Cardio Zone 3–5, Kraft schwer 85–95 % 1RM)', moderat: 'moderat (Cardio Zone 2–3, Kraft 70–80 %, kein Maximalreiz)', ruhe: 'Ruhetag/Regeneration (leichtes Cardio Zone 1–2 oder Pause)' }[verdict.intensity] || verdict.intensity;
    L.push('FAZIT HEUTE (bereits widerspruchsfrei verrechnet aus Werten, Baseline, Frühwarnung und Erholungsphase): ' + verdict.headline + ' – ' + verdict.detail + ' Empfohlene Intensität: ' + intoWords + '.');
    if (verdict.note) L.push('Subjektiver Hinweis (kippt die objektiv gute Erholung nicht, aber erwähnenswert): ' + verdict.note);
    if (verdict.recoveryStale) L.push('Hinweis: Eine ältere Erholungsphase läuft noch, obwohl die heutigen Werte wieder erholt sind – schlage vor, die Pause zu beenden.');
  }
  L.push('GEGENCHECK-PFLICHT: Prüfe die genannten Zahlen (Ruhepuls/HRV gegen die Baseline) gegen das FAZIT. Passen sie nicht zusammen, benenne den Widerspruch freundlich und offen. Gib in JEDER Antwort GENAU EINE klare Empfehlung – niemals zwei sich widersprechende (z. B. „Vollgas" und „Ruhetag" gleichzeitig). Halte dich an das FAZIT. Stelle KEINE Diagnose; werte Auffälligkeiten als Wellness-Signal und verweise bei anhaltenden Beschwerden auf ärztlichen Rat bzw. die Trainer:innen vor Ort.');
...
module.exports = {
  ...
  sanitize, hasAny, baseline, winsorizeHrv, evaluate, readiness, trainReadiness, balance, ageFromRmssd, hrvAge, hrvAgeSeries, trainingZones, subjectiveFlags, trainingLoad, insights, overtraining, recoverySuggest, dayVerdict, weeklyReport, trend, vpLedger, vpLedgerTraining, toPromptText,
