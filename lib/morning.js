...
function dayVerdict(opts) {
  ...
  if (!physioLevel && !alert && !rec) {
    return { level: 'kalibrierung', intensity: 'kalibrierung', headline: 'Kalibrierung läuft', detail: 'Miss ein paar Tage – dann gibt es eine klare Tages-Empfehlung.', source: 'kalibrierung', recoveryStale: false };
  }
  ...
  if (rec && !recoveryStale) {
    return { level: 'rot', intensity: 'ruhe', headline: 'Erholungsphase aktiv', detail: 'Du bist in einer bewussten Erholungsphase – leichte Bewegung, gutes Essen und Schlaf sind jetzt dein Training.', source: 'recovery', recoveryStale: false };
  }
  // Echte Übertrainings-Warnung -> Ruhe (nach dem Ausreißer-Schutz kein Fehlalarm mehr).
  if (alert) {
    return { level: 'rot', intensity: 'ruhe', headline: 'Erholung hat Vorrang', detail: ot.note || 'Deine Erholung liegt seit mehreren Messungen unter deiner Baseline – heute regenerieren.', source: 'alert', recoveryStale: recoveryStale };
  }
  // Sonst: Physiologie-Ampel, nach unten gekappt durch subjektive Signale (Krank/Schlaf/Stress).
  const tl = trainingLoad(r, latest);   // key: voll|moderat|ruhe|kalibrierung
  const intensity = (tl.key === 'kalibrierung') ? 'kalibrierung' : tl.key;
  const level = intensity === 'voll' ? 'gruen' : intensity === 'moderat' ? 'gelb' : intensity === 'ruhe' ? 'rot' : 'kalibrierung';
  const physioKey = physioLevel === 'gruen' ? 'voll' : physioLevel === 'gelb' ? 'moderat' : 'ruhe';
  return { level: level, intensity: intensity, headline: tl.title, detail: tl.detail, source: (tl.key !== physioKey ? 'subjektiv' : 'physio'), recoveryStale: recoveryStale };
}
