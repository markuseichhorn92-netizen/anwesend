...
function evaluate(m, base) {
  ...
  if (m.hrvRmssd != null) {
    if (base.hrv != null) {
      ...
      const note = ratio >= 0.9 ? 'Gute Erholung – dein Nervensystem ist bereit.'
      ...
      push('hrv', 'HRV (RMSSD)', m.hrvRmssd, ' ms', cat, note, 'Dein Schnitt ' + base.hrv + ' ms');
    } else {
...
function trend(list) {
  ...
  const KEYS = ['rhr', 'hrvRmssd', 'respRate', 'orthostaticDelta'];
...
module.exports = {
  ...
  sanitize, hasAny, baseline, evaluate, readiness, trend, vpLedger, toPromptText,
