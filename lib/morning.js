...
const CAP = 180;               // höchstens so viele Messungen behalten (~½ Jahr täglich)
...
// Belastungssteuerung: konkrete Trainings-Empfehlung aus der Ampel. -> { key, title, detail }.
function trainingLoad(r) {
  r = r || {};
  if (!r.level || r.level === 'kalibrierung') return { key: 'kalibrierung', title: 'Baseline wird kalibriert', detail: 'Miss ein paar Tage – dann gibt es eine klare Belastungs-Empfehlung.' };
  if (r.level === 'gruen') return { key: 'voll', title: 'Volle Belastung möglich', detail: 'Dein System ist erholt – heute darfst du intensiv/hart trainieren.' };
  if (r.level === 'gelb') return { key: 'moderat', title: 'Moderat trainieren', detail: 'Teilweise erholt – heute locker bis mittel, kein Maximalreiz.' };
  return { key: 'ruhe', title: 'Regeneration heute', detail: 'Dein Körper steht unter Last – heute Ruhetag, Spaziergang oder lockeres Cardio.' };
}
...
function toPromptText(list) {
  ...
  if (m.sleepSelf) bits.push('Schlaf (Selbstauskunft) ' + m.sleepSelf);
  if (m.moodSelf) bits.push('Befinden ' + m.moodSelf);
...
async function setConsent(id, on) {
  ...
  try {
    ...
    else { await redisPipeline([['DEL', CKEY(id)], ['DEL', MKEY(id)]]); }
    return { ok: true, consent: !!on };
  } catch (e) { return { ok: false, consent: await getConsent(id) }; }
}

module.exports = {
  MKEY, MIN_CALIB, VP_PER_CHECK,
  sanitize, hasAny, baseline, evaluate, readiness, balance, hrvAge, trainingLoad, trend, vpLedger, toPromptText,
  list, add, remove, clear, getConsent, setConsent,
};
