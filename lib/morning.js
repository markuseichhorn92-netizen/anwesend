...
function overtraining(list) {
  ...
  if (base.rhr == null && base.hrv == null) return { level: 'ok', days: 0, note: '' };
  let streak = 0;
  for (let i = 0; i < arr.length; i++) {
    const m = arr[i]; let strained = false;
    if (base.rhr != null && m.rhr != null && (m.rhr - base.rhr) >= 5) strained = true;
    if (base.hrv != null && m.hrvRmssd != null && (m.hrvRmssd / base.hrv) <= 0.85) strained = true;
...
module.exports = {
  ...
  sanitize, hasAny, baseline, evaluate, readiness, trainReadiness, balance, ageFromRmssd, hrvAge, hrvAgeSeries, trainingZones, subjectiveFlags, trainingLoad, insights, overtraining, weeklyReport, trend, vpLedger, vpLedgerTraining, toPromptText,
  list, add, remove, clear, trList, trAdd, trRemove, trClear, getConsent, setConsent, getTeamShare, setTeamShare,
};
