...
async function sendBroadcast(opts) {
  ...
  // Optionaler Deep-Link (z. B. '?fb=1' für die Feedback-Aktion) + Datenfeld, damit
  // ein Tippen auf die Push direkt die passende Stelle in der App öffnen kann.
  const pushUrl = PUBLIC_BASE + '/' + String(opts.query || '');
  const pushData = (opts.data && typeof opts.data === 'object') ? opts.data : null;
  await mapLimited(ids, SEND_SLICE, async (id) => {
    if (wantPush) {
      try {
        const r = await Push.notifyMember(id, 'pushNews', { title: title, body: body, url: pushUrl, data: pushData || undefined });
