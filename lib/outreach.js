 ...
 * Push/E-Mail. Wirft nie.
 * opts: { segment, title, body, channels:{push?, email?} }
 * -> { recipients, pushSent, mailSent, capped }
 */
async function sendBroadcast(opts) {
  ...
  if (!wantPush && !wantEmail) wantPush = true;   // sinnvoller Default: mind. Push

  const result = { recipients: 0, pushSent: 0, mailSent: 0, capped: false };
  ...
  const pushUrl = PUBLIC_BASE + '/mitglieder';
  await mapLimited(ids, SEND_SLICE, async (id) => {
    ...
    if (wantEmail) {
      try { if (await sendMailTo(id, title, body)) result.mailSent++; } catch (e) {}
    }
    return null;
  });
  return result;
 ...
 * opts: { title?, body, channels:{push?, email?} }
 * -> { ok, via:[…] }   via aus 'inbox' | 'push' | 'email'
 */
async function sendDirect(memberId, opts) {
  ...
  if (wantEmail) {
    try {
      if (await sendMailTo(id, title || 'Nachricht vom Fit-Inn', body)) { out.ok = true; out.via.push('email'); }
    } catch (e) {}
  }
  return out;
}
