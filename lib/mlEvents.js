...
// ── Webhook-Telemetrie (NICHT personenbezogen: nur Event-Typ, Zähler, Zeitstempel) ──
// Beantwortet die Betriebsfrage „welche Magicline-Webhooks kommen überhaupt an?".
const STAT = (t) => 'mlx:stat:' + t;
const STAT_TYPES = 'mlx:stat:types';       // SET der je gesehenen Event-Typen
const STAT_LASTTYPE = 'mlx:stat:lasttype';
const STAT_LASTAT = 'mlx:stat:lastat';
const STAT_TTL = 60 * 60 * 24 * 30;        // 30 Tage

async function bumpStat(type) {
  const t = str(type, 60); if (!hasStore || !t) return false;
  try {
    await redisPipeline([
      ['INCR', STAT(t)], ['EXPIRE', STAT(t), String(STAT_TTL)],
      ['SADD', STAT_TYPES, t], ['EXPIRE', STAT_TYPES, String(STAT_TTL)],
      ['SET', STAT_LASTTYPE, t, 'EX', String(STAT_TTL)],
      ['SET', STAT_LASTAT, String(Date.now()), 'EX', String(STAT_TTL)],
    ]);
    return true;
  } catch (e) { return false; }
}
...
async function readStats() {
  const base = { ready: false, types: {}, total: 0, lastType: null, lastAt: null };
  if (!hasStore) return base;
  try {
    const [typeList] = await redisPipeline([['SMEMBERS', STAT_TYPES]]);
    const types = Array.isArray(typeList) ? typeList : [];
    const counts = {}; let total = 0;
    if (types.length) {
      const res = await redisPipeline(types.map((t) => ['GET', STAT(t)]));
      types.forEach((t, i) => { const n = Number(res && res[i]) || 0; counts[t] = n; total += n; });
    }
    const [lt, la] = await redisPipeline([['GET', STAT_LASTTYPE], ['GET', STAT_LASTAT]]);
    return { ready: true, types: counts, total: total, lastType: lt || null, lastAt: la ? Number(la) : null };
  } catch (e) { return base; }
}
