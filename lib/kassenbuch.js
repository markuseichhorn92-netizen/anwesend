...
function computeMonth(m) {
  ...
  return {
    ...
    entriesCount: rows.filter((r) => r.hasEntry).length,
  };
}
...
async function listMonths() {
  ...
  for (const k of keys) {
    ...
    if (!m) continue;
    const t = computeMonth(m);
    out.push({ key: k, year: m.year, month: m.month, closed: !!m.closed, closedAt: m.closedAt || null, updatedAt: m.updatedAt || null, sumEinnahmen: t.sumEinnahmen, sumAusgaben: t.sumAusgaben, endbestand: t.endbestand, entriesCount: t.entriesCount, hasPdf: !!m.closed });
  }
  return out;
}
...
module.exports = {
  ...
  getPrices, setPrices, getMonth, saveMonth, saveCheck, closeMonth, reopenMonth, getPdf, listMonths, suggestOpening, hasStore,
};
