...
const BELEGKEY = (k, d) => 'kb:beleg:' + k + ':' + d;   // gescannter/hochgeladener Kassenbon je Tag
const BELEGIDX = (k) => 'kb:belegidx:' + k;             // Set der Tage mit hinterlegtem Beleg
...
function sanitizeMonth(key, input) {
  ...
  for (let d = 1; d <= 31; d++) {
    ...
    if (s.kaffee || s.eiweissshake || s.eiweissbeutel || s.sonstigesText || s.sonstigesBetrag || s.ausgaben || s.ausgabenText || s.belegOk) days[String(d)] = s;
  }
...
function computeMonth(m) {
  ...
  for (let d = 1; d <= 31; d++) {
    ...
    rows.push({
      ...
      einnahmen: einn, ausgaben: aus, ausgabenText: ausText, belegOk: !!e.belegOk,
...
function toPromptText(month, totals) {
  ...
  const mm = pad(month.month), yy = month.year;
  const L = [];
  L.push('Kassenbuch (Bar-Nebenkasse eines Fitnessstudios) für ' + mm + '/' + yy + '.');
  // Rahmenbedingungen, damit FINN keine bereits geklärten Punkte bemängelt:
  L.push('Jede Buchung ist einem konkreten Kalendertag zugeordnet – das vollständige Datum ist Tag.' + mm + '.' + yy + ' (also z. B. 01.' + mm + '.' + yy + '); die Datumsangabe ist damit vollständig.');
  L.push('Umsatzsteuer: pauschal 19 % auf alle Barverkäufe (keine 7/19-%-Aufteilung).');
  L.push('Belege werden je Buchung mit einem Häkchen dokumentiert (unten als „Beleg: ja/nein"). Ein Kassensturz (gezählter Bestand) ist freiwillig.');
  ...
  const buchungen = t.rows.filter((r) => r.hasEntry).map((r) => pad(r.day) + '.' + mm + '.' + yy + ': ' + (r.beschreibung || '—') + (r.einnahmen ? (' | Einnahme ' + r.einnahmen + ' €') : '') + (r.ausgaben ? (' | Ausgabe ' + r.ausgaben + ' €' + (r.ausgabenText ? (' (' + r.ausgabenText + ')') : '') + ' | Beleg: ' + (r.belegOk ? 'ja' : 'nein')) : ''));
...
async function saveCheck(key, check) {
  ...
  m.check = {
    ...
    findings: (check && Array.isArray(check.findings)) ? check.findings.slice(0, 12) : [],
    text: String((check && check.text) || '').slice(0, 2000),
  };
...
// ── Belege (gescannte/hochgeladene Kassenbons je Buchungstag) ──
// Bild/PDF als data-URL-String, studioweit unter kb:beleg:<YYYY-MM>:<Tag>. KEIN Personenbezug.
function belegDay(d) { const n = parseInt(d, 10); return (n >= 1 && n <= 31) ? n : 0; }
async function saveBeleg(key, day, dataUrl) {
  const d = belegDay(day);
  if (!isMonthKey(key) || !d) return { ok: false, error: 'bad_input' };
  const s = String(dataUrl || '');
  if (!/^data:(image\/(jpeg|png|webp)|application\/pdf);base64,/.test(s)) return { ok: false, error: 'bad_image' };
  if (s.length > 1.6e6) return { ok: false, error: 'too_large' };   // ~1,1 MB dekodiert
  if (!hasStore) return { ok: true, disabled: true };
  try { await redisPipeline([['SET', BELEGKEY(key, d), s], ['SADD', BELEGIDX(key), String(d)]]); return { ok: true }; }
  catch (e) { return { ok: false, error: 'store' }; }
}
async function getBeleg(key, day) {
  const d = belegDay(day);
  if (!isMonthKey(key) || !d || !hasStore) return null;
  try { const [v] = await redisPipeline([['GET', BELEGKEY(key, d)]]); return v || null; } catch (e) { return null; }
}
async function delBeleg(key, day) {
  const d = belegDay(day);
  if (!isMonthKey(key) || !d) return { ok: false };
  if (!hasStore) return { ok: true, disabled: true };
  try { await redisPipeline([['DEL', BELEGKEY(key, d)], ['SREM', BELEGIDX(key), String(d)]]); return { ok: true }; } catch (e) { return { ok: false }; }
}
async function listBelege(key) {
  if (!isMonthKey(key) || !hasStore) return [];
  try { const [v] = await redisPipeline([['SMEMBERS', BELEGIDX(key)]]); return (v || []).map((x) => parseInt(x, 10)).filter((n) => n >= 1 && n <= 31).sort((a, b) => a - b); }
  catch (e) { return []; }
}
...
// Einen Monat UNBEDINGT (über)schreiben – für den Abgleich eines Archiv-/Import-Monats
// mit den geprüften Seed-Werten (falls ein bereits importierter Monat später korrigiert
// wurde). Nur für Import-Archive verwenden; überschreibt vorhandene Daten vollständig.
async function overwriteMonth(rec) {
  const key = rec && rec.key;
  if (!isMonthKey(key) || !hasStore) return { ok: false };
  try {
    await redisPipeline([['SET', MKEY(key), JSON.stringify(Object.assign({}, rec, { updatedAt: new Date().toISOString() }))], ['SADD', IDX_KEY, key]]);
    return { ok: true };
  } catch (e) { return { ok: false }; }
}

// Bereits importierte Archiv-Monate mit den GEPRÜFTEN Seed-Werten abgleichen.
// Der Import überspringt vorhandene Monate – wird ein Seed-Wert später korrigiert
// (z. B. Juni von falschen 283,50 € auf korrekte 777,00 €), bleibt der alte gespeicherte
// Wert sonst für immer stehen. Deshalb überschreiben wir einen abweichenden Import-Archiv-
// Monat mit dem Seed. NUR echte Import-Archive (source==='import'); manuell geführte Monate
// bleiben unberührt. Idempotent – schreibt nur bei echter Abweichung. -> Anzahl korrigierter Monate.
async function reconcileSeed() {
  if (!hasStore) return 0;
  let SEED; try { SEED = require('./kassenbuchSeed'); } catch (e) { return 0; }
  const rows = (SEED && Array.isArray(SEED.SEED_2026)) ? SEED.SEED_2026 : [];
  const close = (a, b) => Math.abs((Number(a) || 0) - (Number(b) || 0)) < 0.005;
  let fixed = 0;
  for (const s of rows) {
    try {
      const stored = await getMonth(s.key);
      if (!stored || stored.source !== 'import') continue;   // nur seed-importierte Archive
      const im = stored.imported || {};
      const same = close(im.sumEinnahmen, s.sumEinnahmen) && close(im.sumAusgaben, s.sumAusgaben)
        && close(im.endbestand, s.endbestand) && close(stored.anfangsbestand, s.anfangsbestand)
        && String(stored.blattNr || '') === String(s.blattNr);
      if (!same) { const r = await overwriteMonth(SEED.seedRecord(s)); if (r && r.ok) fixed++; }
    } catch (e) {}
  }
  return fixed;
}
...
module.exports = {
  ...
  getPrices, setPrices, getMonth, saveMonth, saveCheck, closeMonth, reopenMonth, getPdf, listMonths, suggestOpening, importMonth, overwriteMonth, reconcileSeed, hasStore,
  saveBeleg, getBeleg, delBeleg, listBelege,
};
