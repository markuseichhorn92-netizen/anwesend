...
function buildFoods(rows, blsArr) {
  ...
  curated.forEach(function (f) { f.terms.forEach(function (t) { covered.add(t); }); });
  ...
  (Array.isArray(blsArr) ? blsArr : []).forEach(function (b, i) {
    ...
    const nName = norm(name), nBase = norm(baseName);
    if (!nName || covered.has(nName) || covered.has(nBase)) return;   // schon kuratiert
    covered.add(nName);
    out.push({
 ...
 * - danach OFF-Produkte, deren Name nicht schon durch ein Grundnahrungsmittel
 *   abgedeckt ist (verhindert doppelte „Banane"),
...
function mergeWithOff(baseProducts, offProducts, limit) {
  ...
  const out = base.slice(0, lim);
  for (const p of off) {
    if (out.length >= lim) break;
    const n = norm(p && p.name);
    if (!n || covered.has(n)) continue;   // schon durch Grundnahrung abgedeckt
    covered.add(n);
    out.push(p);
  }
