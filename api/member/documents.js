...
/**
 * /api/member/documents   (Authorization: Bearer <token>)
 * Dokumente des angemeldeten Mitglieds (Scopes CUSTOMER_DOCUMENT_READ /
 * CUSTOMER_DOCUMENT_WRITE) - über die 403-festen Wrapper in lib/mlDocuments.
 *
 *   GET               -> Liste:   { ok:true, available, items:[...] }
 *   GET ?docId=...    -> Download: Rohbytes (Content-Disposition) ODER
 *                                  { ok:true, url } (Redirect-Link) ODER 404 JSON
 *   POST { name, contentType, base64 } -> Upload
 *
 * Graceful Degradation ist zentral: liefert die API nicht sauber, gilt
 * available:false und die UI blendet den Bereich aus bzw. fällt auf den
 * bestehenden Weg zurück (z. B. /api/member/contract-copy). Nie werfen.
 */

const M = require('../../lib/members');
const D = require('../../lib/mlDocuments');
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    let docId = null, wantName = null;
    try { const u = new URL(req.url, 'http://x'); docId = u.searchParams.get('docId'); wantName = u.searchParams.get('name'); } catch (e) {}

    if (docId) {
      let doc;
      try { doc = await D.getDocument(sess.id, docId); } catch (e) { doc = { available: false }; }
    ...
    let list;
    try { list = await D.listDocuments(sess.id); } catch (e) { list = { available: false }; }
  ...
  if (req.method === 'POST') {
    ...
    // Leichter Spam-Schutz: max. 10 Uploads pro Stunde.
    if (!(await M.rateLimit('doc-upload:' + sess.id, 10, 3600))) {
      res.statusCode = 200; return res.end(JSON.stringify({ ok: false, message: 'Zu viele Uploads. Bitte später erneut.' }));
    }

    let r;
    try { r = await D.uploadDocument(sess.id, { name: name, contentType: contentType, base64: base64 }); }
    catch (e) { r = { ok: false, forbidden: false }; }

    if (r && r.ok) {
      res.statusCode = 200; return res.end(JSON.stringify({ ok: true, message: 'Dokument hochgeladen. Danke!' }));
    }
    if (r && r.forbidden) {
      res.statusCode = 200;
      return res.end(JSON.stringify({ ok: false, forbidden: true, message: 'Das Hochladen ist über den Mitgliederbereich aktuell nicht möglich. Bitte bring das Dokument vorbei oder schick es uns per E-Mail.' }));
    }
    res.statusCode = 200;
